...
import request, { authRequest } from 'rc-mobile-base/lib/utils/request';
import { roomsSelector } from 'rc-mobile-base/lib/selectors/rooms';
import OutboundActions from '../actions/outbound';
import _ from "lodash"
import { updateAuditImageUrlApi } from "rc-mobile-base/lib/actions/updates"
const API_URL = `/Audit/GetListOfTemplates`
...
function* generateUploadedImageArray(payload) {
  const stringifyData = JSON.stringify(payload)
  const data = JSON.parse(stringifyData)
  let imageUrlArray = []
  for (let [mainIndex, item] of data.sections.entries()) {
    for (let [index, que] of item.questions.entries()) {
      if (data.sections[mainIndex].questions[index].answerImageUrls && !_.isEmpty(data.sections[mainIndex].questions[index].answerImageUrls) ) {
        for (let [urlIndex, urlItem] of que.answerImageUrls.entries()) {
          const imageUrl = yield call(uploadPhoto, urlItem.value)
          imageUrlArray.push(imageUrl)
          if (que.answerImageUrls.length === (urlIndex + 1)) {
            data.sections[mainIndex].questions[index].answerImageUrls = imageUrlArray
            imageUrlArray = []
          }
        }
      }
    }
    if (data.sections.length === (mainIndex + 1)) {
      return data
    }
  }
...
class Audit extends Resource {
  ...
  onInsert = function* ({ payload }) {
    try {
      ...
      //   })
      // });
      yield put(OutboundActions.auditInsert({ audit: data, inspections }));
  ...
  onUpdate = function* ({ payload }) {
    try {
      const { roomAudit: { insertAuditRouteId } } = yield select();
      const { inspections, ...data } = payload

      const { imageUrl } = data;

      const uploadedImage = yield call(generateUploadedImageArray, payload)
      ...
      //   })
      // });
      // yield put(OutboundActions.auditTemplateUpdate({ audit: { ...uploadedImage, id: insertAuditRouteId } }));
      yield put(OutboundActions.createAuditInsertRoute({ audit: { ...uploadedImage, id: insertAuditRouteId } }));
  ...
  onDelete = function* () {
    try {
      const { roomAudit: { insertAuditRouteId } } = yield select();
      yield put(OutboundActions.deleteAuditApi(insertAuditRouteId));

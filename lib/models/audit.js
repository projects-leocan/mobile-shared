...
function* generateUploadedImageArray(payload) {
  ...
  for (let [mainIndex, item] of data.sections.entries()) {
    for (let [index, que] of item.questions.entries()) {
      if (data.sections[mainIndex].questions[index].answerImageUrls) {
        const imageUrl = yield call(uploadPhoto, que.answerImageUrls)
        data.sections[mainIndex].questions[index].answerImageUrls = [imageUrl]
      }
    }

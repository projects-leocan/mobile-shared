...
const validateFilterStatus = (filterOperation) => {
  try {
    ...
    if (isRoomOperation || isFloorOperation || isFloorSectionOperation || isRoomTypeOperation || cleanerSelectionOperation || cleanerSelectionOperation ) {
      return true
    } else {
      return false
    }
...
const ACTION_HANDLERS = {
  ...
  [InspectorFiltersTypes.UPDATE_CLEANER_SELECTION]: (state, { data }) => {
    const updatedFilterOperation = {
      ...
      // roomsSearchQuery:state.roomsSearchQuery
    }
    return {
      ...state,

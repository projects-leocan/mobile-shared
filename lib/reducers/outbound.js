...
const ACTION_HANDLERS = {
  ...
  [OutboundTypes.OUTBOUND_ROOM_CLEAN_FAILURE]: (state, { type, meta, payload }) => {
   ...
   [OutboundTypes.OUTBOUND_CENCLE_CLEANING_FAILED]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [
  ...
  [OutboundTypes.OUTBOUND_ASSIGN_CLEANING_FAILED]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [
  ...
  [OutboundTypes.OUTBOUND_RE_ASSIGN_CLEANING_FAILED]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [
  ...
  [OutboundTypes.OUTBOUND_LOCATOR_STATUS_FAILURE]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [
  ...
  [OutboundTypes.OUTBOUND_FOUND_UPDATE_PHOTO_FAILURE]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [
  ...
  [OutboundTypes.OUTBOUND_ROOM_UPDATE_FAILURE]: (state, { type, meta, payload }) => {
    ...
    return {
      ...state,
      failedRequests: meta.completed === true ? [] : [
        ...state.failedRequests,
        { type, meta, errorMessage: message || "" }
      ]
    }
  ...
  [RoomsTypes.RESERVATION_CAME_TO_BREAKFAST_FAILURE]: (state, { type, meta, payload }) => {
    const { message } = payload;
    return {
      ...state,
      failedRequests: [

...
const ACTION_HANDLERS = {
  ...
  [UpdatesTypes.INVENTORY_ADJUST]: (state, { roomId, assetRoomId, updateInventoryData }) => {
  ...
  [UpdatesTypes.NIGHT_ROOM_CLEAN_START]: (state, { roomId }) => {
    const { rooms } = state;
    console.log("ROOM_TURNDOWN updatesTypes rooms",rooms);
    console.log("ROOM_TURNDOWN updatesTypes roomId",roomId);
  ...
  [UpdatesTypes.UPDATE_CLEANING_START_VALUE]: (state, { roomId , ts }) => {
    const newEntry = {
      roomId,
      startTime: ts,
      endTime: null,
  ...
  [UpdatesTypes.TASKS_ADD_RECENT]: (state, { data }) => {
    let tasksRecent = state.tasksRecent && [ ...state.tasksRecent ] || [];
    
    if (tasksRecent > 2) {
      tasksRecent.shift();
    }
    
    tasksRecent.unshift(data);

...
const getInitialState = () => ({
  ...
  tasks: { lastUpdate: null },
  assets: { lastUpdate: null },
  messages: { lastUpdate: null },
  availHotelsTask: { lastUpdate: null },
  inspectionClean: { lastUpdate: null },
  virtualAssets: { lastUpdate: null },
  durableAssets: { lastUpdate: null },
  roomAreas: { lastUpdate: null },
  customActions: { lastUpdate: null },
...
const ACTION_HANDLERS = {
  ...
  [BackendTypes.ROOM_MESSAGES_FETCHED]: (state) => {
    // return state.setIn(['assets', 'lastUpdate'], moment().unix());
    return {
      ...state,
      messages: { lastUpdate: moment().unix() }
    }
  },
  [BackendTypes.AVAIL_HOTELS_TASK_FETCHED]: (state) => {
    // return state.setIn(['assets', 'lastUpdate'], moment().unix());
    return {
      ...state,
      availHotelsTask: { lastUpdate: moment().unix() }
    }
  },
  [BackendTypes.ROOM_INSPECTION_CLEAN_FETCHED]: (state) => {
    // return state.setIn(['assets', 'lastUpdate'], moment().unix());
    return {
      ...state,
      inspectionClean: { lastUpdate: moment().unix() }
    }
  },

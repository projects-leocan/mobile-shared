...
const ACTION_HANDLERS = {
  ...
  [RoomsTypes.SINGLE_CALENDER_SUCCESS]: (state, {reservation}) =>{
    let singleReservation = !_.isEmpty(reservation) ? get(reservation, 0, {}) : {}
    const index = !_.isEmpty(state.hotelCalendar) && state.hotelCalendar.findIndex((reservationData) => reservationData.id === singleReservation.id);
    let updatedHotelCalendar = [...state.hotelCalendar];
    if (index !== -1) {
      updatedHotelCalendar[index] = singleReservation;
    } else {
      updatedHotelCalendar.push(singleReservation);
    }
    return{
      ...state,
      hotelCalendar: updatedHotelCalendar,
    }
  },

  [RoomsTypes.REMOVE_RESERVATION]: (state, {results}) => {
    let idsToRemove = !_.isEmpty(results) ? results : []
    const filteredArray = !_.isEmpty(state.hotelCalendar) && state.hotelCalendar.filter(item => !idsToRemove.includes(item.id));   
    return {
      ...state,
      hotelCalendar: filteredArray,
    }
  },
  ...
  [RoomsTypes.TASKS_SUCCESS]: (state, { tasks }) => {
   ...
   if (_.isEmpty(tasks)) {
    ...
    filteredTasks = tasks.filter(task => !task.is_cancelled);
   }

    return {
      ...state,
      hotelTasks: filteredTasks
    }
  },

  [RoomsTypes.AUDIT_TASKS_SUCCESS]: (state, { tasks }) => {
    let getTask = !_.isEmpty(tasks) ? get(tasks, 0, {}) : {}
    let filteredTasks = []
    if (_.isEmpty(getTask)) {
     filteredTasks = state.hotelTasks
    }else{
      filteredTasks = !_.isEmpty(state.hotelTasks) && state.hotelTasks.map((task) =>{
        return task.id !== getTask.id ? task : {
          ...task,
          is_audit_required: getTask.is_audit_required,
          audit_template_id: getTask.audit_template_id,
          audit_id: getTask.audit_id,
          is_audit_answered: getTask.is_audit_answered
      };
      })
    }
     return {
  ...
  [RoomsTypes.CANCEL_TASKS]: (state, { cancelTasks }) => {
    let idsToRemove = !_.isEmpty(cancelTasks) ? cancelTasks : []
    const filteredArray = !_.isEmpty(state.hotelTasks) && state.hotelTasks.filter(item => !idsToRemove.includes(item.id));

    return {
      ...state,
      hotelTasks: filteredArray
    }
  },
  ...
  // [OutboundTypes.OUTBOUND_INVENTORY_ASSET_CONSUMATION_SUCCESS]: (state, action) => {
  //   const { payload, meta } = action;
  //   console.log("OUTBOUND_INVENTORY_ASSET_CONSUMATION_SUCCESS action +++", action);
  // },

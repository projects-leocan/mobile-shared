...
const VipContainer = ({ roomName, vip, source, memberShip, is_baby }) => (
  (vip || ((source && source.value === "ClubMed" && is_baby === 1) || (source.value === "ClubMed" && memberShip?.value !== "UNSUPPORTED TYPE: Null"))) &&
  <View style={styles.infoRow}>
      ...
      source ?
        source.value === "ClubMed" ?
          <View style={styles.ClubMedView}>
            <Text style={[styles.pms, { alignSelf: "center", paddingRight: 10 }]}>{memberShip?.value === "UNSUPPORTED TYPE: Null" ? "" : memberShip?.value}</Text>

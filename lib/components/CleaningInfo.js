          ...
          source ?
            source.value === "ClubMed" ?
              <View style={styles.ClubMedView}>
                {memberShip?.value !== "UNSUPPORTED TYPE: Null" &&
                ...
                {is_baby === 1 ? <Image style={styles.isbabyImageView} source={require("../images/baby.png")} /> : <></>}
                {serviceGo?.value !== "UNSUPPORTED TYPE: Null" && <Text style={[styles.pms, { alignSelf: "center", paddingRight: 10 }]}> {serviceGo?.value}</Text>}
                {groupName?.value === "UNSUPPORTED TYPE: Null" ? "" : <Text style={[styles.pms, { alignSelf: "center" }]}>{groupName}</Text>}
              </View>
              : <></> : <></>
        }
      </View>}
...
const CleaningInfo = ({ room, activeId = null, config = {}, isTurndown = false, isRunner = false, style, inline = false, isAttendantApp, hotel_group_key }) => {
  ...
  let memberShip = activeRoomCalender[0]?.otherProperties?.filter((data) => {
    if (data.key === "Membership") {
      return data.value
    }
  })

  let source = activeRoomCalender[0]?.otherProperties?.filter((data) => {
    if (data.key === "source") {
      return data.value
    }
  })

  let serviceGo = activeRoomCalender[0]?.otherProperties?.filter((data) => {

    ...
    if (GuestStatus === 'ARR') {
      return (
        <>
          ...
          <Text style={[styles.etInfoText, { marginRight: 35 }]}>{eta && unixPrettyTime(eta)}</Text>
        </>
      )
    } else {
      return <></>
...
const CLEANINGNAME = ({ cleaningName, credits, isChangeSheets }) => (
  <View style={styles.infoRow}>
    <Text style={styles.pms}>{cleaningName} {`${credits ? `(${credits})` : ''}`}</Text>
    {isChangeSheets
      ?
      <ChangeSheets />
      : null
...
const CleaningInfo = ({ room, activeId = null, config = {}, isTurndown = false, isRunner = false, style, inline = false, isAttendantApp }) => {
  if (!room) {
    return null;
  }
      ...
      <View style={{ marginTop: 15 }}>
        ...
        <View style={[styles.infoContainer, inline && { marginLeft: 0, marginRight: 0, paddingHorizontal: 5, backgroundColor: 'white' }]}>
          ...
          {cleaningName.length > 0
            ?
            <CLEANINGNAME cleaningName={cleaningName} credits={credits} isChangeSheets={isChangeSheets} />
            : null
          }

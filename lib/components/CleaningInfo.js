...
import { getColorBasedOnMemberShipValue, getWidthOfImageBaedOnCondition } from 'rc-mobile-base/lib/utils/calendar';
...
const Vip = ({ vip }) => (
  <View style={[styles.vipContainer, { alignSelf: "center" }]}>
...
const Traces = ({note}) =>(
  <View style={styles.infoRow}>
  <View style={styles.iconContainer}>
    <Text style={[styles.pms]}>Traces:</Text>
  </View>
  <Text style={styles.infoText} numberOfLines={6}>{note}</Text>
</View>
)
...
const ETView = ({ guestStatus, eta, etd, guest }) => {
                ...
                <View style={[styles.infoRow, { justifyContent: 'flex-start', alignItems: 'flex-start' }]}>
                  {gue?.status === "departed" || gue?.status === "departure" ?
                    <>
                      ...
                      <Text style={[styles.etInfoText, { marginRight: 35 }]}>{gue?.etdTime}</Text>
                    </>
                    :
                    gue?.status === "arrival" || gue?.status === "arrived" ?
                      <>
                        ...
                        <Text style={[styles.etInfoText, { marginRight: 35 }]}>{gue?.etaTime}</Text>
                      </>
                      : null}
                </View>
              )
    ...
    if (GuestStatus === 'ARR') {
      return (
        <>
          ...
          <Text style={[styles.etInfoText, { marginRight: 35 }]}>{eta && eta}</Text>
        </>
      )
    } else {
      return <></>
    ...
    if (GuestStatus === 'DEP') {
      return (
        <>
          ...
          <Text style={styles.infoText}>{etd && etd}</Text>
        </>
      )
    } else {
      return <></>
...
const VipContainer = ({ roomName, vip, source, memberShip, is_baby, serviceGo, groupName }) => (
  <>
    {(vip || ((source && source?.value === "ClubMed" && is_baby && is_baby === 1) || (source?.value === "ClubMed" && memberShip && memberShip?.value !== "UNSUPPORTED TYPE: Null" && (memberShip?.value.toLowerCase() === 'silver' || memberShip?.value.toLowerCase() === 'turquoise' || memberShip?.value.toLowerCase() === 'platinium' || memberShip?.value.toLowerCase() === 'platinum' || memberShip?.value.toLowerCase() === 'gold')) || (source?.value === "ClubMed" && serviceGo && serviceGo?.value !== "UNSUPPORTED TYPE: Null") || (source?.value === "ClubMed" && groupName && groupName !== "UNSUPPORTED TYPE: Null"))) &&
      <View style={styles.infoRow}>
        ...
        {vip
          ...
          <>
            <Vip vip={vip} />
            <Text style={[styles.pms, { paddingHorizontal: 10 }]}>{vip}</Text>
          </>
          : null
        }
        {
          source ?
            source?.value === "ClubMed" ?
              <View style={styles.ClubMedView}>
                {memberShip && memberShip?.value !== "UNSUPPORTED TYPE: Null" &&
                  <Image style={[styles.isbabyImageView, { minWidth: getWidthOfImageBaedOnCondition(memberShip?.value) }]} source={getColorBasedOnMemberShipValue(memberShip?.value)} resizeMode="contain" />}
                {is_baby && is_baby === 1 ? <Image style={styles.isbabyImageView} source={require("../images/baby.png")} /> : <></>}
                {serviceGo && serviceGo?.value !== "UNSUPPORTED TYPE: Null" && <Text style={[styles.pms, { alignSelf: "center", paddingRight: 10 }]}> {serviceGo?.value}</Text>}
                {groupName && groupName?.value === "UNSUPPORTED TYPE: Null" ? "" : <Text style={[styles.pms, { alignSelf: "center" }]}>{groupName}</Text>}
              </View>
              : <></> : <></>
        }
      </View>}
  ...
  <View style={styles.infoRow}>
    ...
    <View style={{ flexDirection: 'column' }}>
      {messages.map((item, index) => {
        return <Text key={index.toString()} style={[styles.etInfoText, { fontSize: 16 }]} numberOfLines={6}>{item}</Text>
      })}
    </View>
  </View>
)
...
const CleaningInfo = ({ room, activeId = null, config = {}, isTurndown = false, isRunner = false, style, inline = false, isAttendantApp, hotel_group_key }) => {
  ...
  let selectedGuest = guests && guests.length && guests[0];
  let guestOccupants = selectedGuest && selectedGuest.occupants;
  ...
  let traces = !_.isEmpty(activeRoomCalender) && activeRoomCalender[0]?.otherProperties?.filter((data) => {
    if (data?.key === "traces") {
      return data?.value
    }
  })
  ...
  let serviceGo = !_.isEmpty(activeRoomCalender) && activeRoomCalender[0]?.otherProperties?.filter((data) => {
    if (data?.key.toLowerCase() === "service_go") {
      return data?.value
    }
  })

  let is_baby = get(activeRoomCalender[0], 'infants', 0)
  let groupName = get(activeRoomCalender[0], 'group_name', 0)
  ...
  if (activeId) {
    ...
    let foundActiveOtherProperties = foundActivePms.otherProperties
    pmsNote = foundActivePms ? get(foundActivePms, 'pms_note') : pmsNote;
     !_.isEmpty(foundActiveOtherProperties) && foundActiveOtherProperties.filter((data) =>{
      if (data?.key === "traces") {
        traces = data.value
      }
    })
    ...
    eta = foundActiveGuest ? ['arrived', 'arrival'].includes(get(foundActiveGuest, 'status')) && get(foundActiveGuest, ['etaTime']) : etaTime;
    etd = foundActiveGuest ? ['departed', 'departure'].includes(get(foundActiveGuest, 'status')) && get(foundActiveGuest, ['etdTime']) : etdTime;
  ...
  if (!isChangeSheets && !vip && !pmsNote && !message && !roomCategory && !credits && !traces) {
    return null;
  }
    ...
    <View style={[styles.container, style]}>
      <View style={{ marginTop: 15 }}>
        ...
        <View style={[styles.infoContainer, inline && { marginLeft: 0, marginRight: 0, paddingHorizontal: 5, backgroundColor: 'white' }]}>
          ...
          {cleaningName && cleaningName.length > 0
            ?
          ...
          {
            !_.isEmpty(roomCalendar) &&
            <VipContainer
              serviceGo={!_.isEmpty(serviceGo) && serviceGo[0]}
              source={!_.isEmpty(source) && source[0]}
              memberShip={!_.isEmpty(memberShip) && memberShip[0]}
              is_baby={is_baby}
              vip={vip}
              groupName={groupName}
            />
          ...
          {
            !_.isEmpty(traces) ? <Traces note={traces}></Traces> : null
          }
          ...
          {(eta || etd) ?
            <ETView guestStatus={guestStatus} eta={etaTime} etd={etdTime}  ></ETView>
            : null
          }

          {
            String(guestStatus).toUpperCase() === "DA" ?
              <RenderEtaEtdBoth guestStatus={guestStatus} guests={guest} />
              : null
          }

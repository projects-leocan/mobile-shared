...
class AssignmentSelect extends Component {
  ...
  _handleSelectOption = (option, data) => {
    const { selectedOptions, selectedObject } = this.state;
  ...
  render() {
    ...
    const { searchQuery, selectedOptions, selectedObject } = this.state;
      ...
      <View style={styles.container}>
        ...
        <View style={[flx1]}>
          <ListView
            ...
            renderSectionHeader={(label) => <Header row={label} />}
            getSectionId={(userRow) => userRow.type}
          />
        </View>
        <View style={[flxRow]}>
          <Button style={[{ height: 50, borderRadius: 0, opacity: _.isEmpty(selectedObject) ? 0.5 : 1 }, green.bg, flx4, lCenterCenter]} onPress={() => _.isEmpty(selectedObject) ? null : handleSubmit(selectedObject)}>

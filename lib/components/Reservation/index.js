...
import { last, first, take } from 'lodash/array';
  ...
  render() {
    ...
    const allItems = get(room, ['roomCalendar']).sort(dateSort)
      .map((entry, index) => {
        ...
        if (roomStatus === 'da') {
          ...
          const arrGuest = get(allGuests, 'ARR', []);
    ...
    return (
      <View style={[style]}>
        ...
        {isHorizontal && sortReservationData.length > 1 ?
          <ScrollView horizontal contentContainerStyle={{ height: 100, marginBottom: 10 }}>
            {sortReservationData.map((reservation, index) =>
              <Entry
                key={index}

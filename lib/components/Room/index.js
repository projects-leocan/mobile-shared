  ...
  renderLocatorStatus = (roomGuests, isGuestIn) => {
    if (roomGuests) {
      ...
      if (GuestStatus === 'STAY') {
        if (!isGuestIn) {
          return (
            <Icon name="user" size={28} color={isGuestIn ? red.color : green.color} style={{ marginRight: 20, alignSelf: 'center', alignSelf: 'center', textAlign: 'center' }} />
          )
        }
      } else if (GuestStatus === 'DEP') {
        return (
          <Entypo name="log-out" color={isGuestIn ? red.color : green.color} size={28} style={{ marginRight: 20, textAlign: 'center', alignSelf: 'center' }} />
        )

...
export default class Room extends React.Component {
  ...
  render() {
    ...
    const finalObject = {
      memberShip: get(first(memberShip), 'value', "").toLowerCase() === 'gold' || get(first(memberShip), 'value', '').toLowerCase() === 'platinium',
      infants: get(first(activeRoomCalender), 'infants', 0) === 1,
    }
              ...
              activeRoomCalender[0]?.otherProperties?.map((data) => {
                return data.key === "source" && data.value === "ClubMed" ?
                  <View style={{ flex: 1, flexDirection: "row" }}>
                    {
                      activeRoomCalender[0]?.infants === 1 ? <Image style={{ height: 30, width: 30, marginRight: 20 }} source={require("../../images/baby.png")} /> : <></>
                    }

...

            {
              activeRoomCalender[0]?.otherProperties?.map((data) => {
                return data.key === "source" && data.value === "ClubMed" ?
                  <SpecialInfoContainer>
                    <Image style={{ height: 30, width: getWidthOfImageBaedOnCondition(memberShip[0]?.value) }} source={getColorBasedOnMemberShipValue(memberShip[0]?.value)} resizeMode="contain" />
              ...
              activeRoomCalender[0]?.otherProperties?.map((data) => {
                return data.key === "source" && data.value === "ClubMed" ?
                  <View style={{ flex: 1, flexDirection: "row" }}>
                    ...
                    <MemberShipText style={{ color: skyDK, alignSelf: "center" }}>{groupName === "UNSUPPORTED TYPE: Null" ? "" : groupName}</MemberShipText>
                  </View>
                  :
                  <></>
              })

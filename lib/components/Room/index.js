...
export default class Room extends React.Component {
  ...
  render() {
    ...
    let checkForIsBaby = !_.isEmpty(roomCalendar) && roomCalendar.some((d) => d.infants)
    const finalObject = {
      memberShip: !_.isEmpty(roomCalendar) && roomCalendar?.some((d) => d?.otherProperties?.some((p) => (p?.key === 'Membership' && p?.value?.toLowerCase() === 'gold') || (p?.key === 'Membership' && p?.value?.toLowerCase() === 'platinium'))),
      infants: !_.isEmpty(roomCalendar) && roomCalendar?.some((d) => d?.infants),
    }
    ...
    return (
      <Base onPress={() => roomNavigation(room.id)} style={{
        ...
        <CenterView>
          ...
          <InnerView style={{ flexDirection: "row", justifyContent: "flex-start", paddingLeft: 100 }}>
            ...
            {
              activeRoomCalender[0]?.otherProperties?.map((data) => {
                return data.key === "source" && data.value === "ClubMed" ?
                  <SpecialInfoContainer>
                    <Image style={{ height: 30, width: getWidthOfImageBaedOnCondition(memberShip[0]?.value), marginRight: 20 }} source={getColorBasedOnMemberShipValue(memberShip[0]?.value)} resizeMode="contain" />
                    {/* <MemberShipText style={{ backgroundColor: getColorBasedOnMemberShipValue(memberShip[0]?.value), padding: 5 }}>{memberShip[0]?.value === "UNSUPPORTED TYPE: Null" ? "" : memberShip[0]?.value}</MemberShipText> */}
                  </SpecialInfoContainer>
                  :
                  <></>
              })
              ...
              activeRoomCalender[0]?.otherProperties?.map((data) => {
                return data.key === "source" && data.value === "ClubMed" ?
                  <View style={{ flex: 1, flexDirection: "row" }}>
                    ...
                    <MemberShipText style={{ color: skyDK }}>{groupName[0]?.value === "UNSUPPORTED TYPE: Null" ? "" : groupName[0]?.value}</MemberShipText>

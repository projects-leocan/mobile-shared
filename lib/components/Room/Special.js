...
export default Special = ({ tasks = 0, scheduledTs = null, scheduledOrder = null, isFinished = false, showCleaningOrderValue = false }) => {
  let label, color;

  if (showCleaningOrderValue == true) {
    label = scheduledOrder;
    color = orange.color;
  } else if (showCleaningOrderValue == false) {

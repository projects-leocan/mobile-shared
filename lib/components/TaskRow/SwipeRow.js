      ...
      <ModalToggler
        ...
        renderValue={(toggle) => (
          <Swipeable
            ...
            style={[flex1]}
            rightButtons={validateActivities.map(activity => createSwipeOption(activity, toggle, task))}
          >
            <Row {...this.props} task={task} />
          </Swipeable>
        )}
        renderModal={(toggle) => {
          const withChildren = validateActivities.find(activity => activity.children)
          if (!withChildren) {
            return null
          }
          return (

...
class SwipeRow extends Component {
  ...
  _handlePress = (activity, toggle) => {
    ...
    if(taskType === 'quick' && is_claimed && get(task, 'task', '')) {
  ...
  render() {
    ...
    const createSwipeOption = (activity, toggle, task) => {
      ...
      if (taskType === 'quick' && is_claimed && get(task, 'task', '')) {
        return (
          <TouchableOpacity style={[{ height: 70, backgroundColor: green.color }, flex1, { justifyContent: 'center', marginVertical: 8 }]} onPress={() => this._handlePress(activity)}>
            <Text style={[white.text, { width: 75, textAlign: 'center' }]}>Finish</Text>
          </TouchableOpacity>
        )
      } else {
        return (
          <TouchableOpacity style={[{ backgroundColor: activity.backgroundColor, height: 70 }, flex1, { justifyContent: 'center', marginVertical: 8 }]} onPress={() => this._handlePress(activity)}>

...
export const TaskRow = ({ task, onPress }) => (
  console.log("get(task, 'guests', {}) +++",task),
  <TouchableOpacity
    ...
    <View style={styles.container}>
      ...
      <View style={{ width: "45%", alignItems: "flex-start", justifyContent: "space-around" }}>
        <View style={styles.titleView}>
          <Title value={task.task} message={get(task, 'comment', '')} isGuest={get(task, 'is_guest_request', false)} isPriority={get(task, 'is_priority', false)} typeKey={get(task, 'typeKey' , null)}/>
        </View>
        ...
        /> */}
        <Guest
          guests={get(task, 'guests', {})}
          roomName={task.singleRoom.name}
          isGuestIn={task.singleRoom.isGuestIn}
          roomHousekeeping={task.roomHousekeeping}
          guestStatus={task.guestStatus}
        />
      </View>

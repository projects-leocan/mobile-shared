...
const renderRow = (onUpdate, onNavigateAudit) => (props) => {
  if (props.withOptions) {
    return (<RowWithOptions onUpdate={onUpdate} {...props} />)
  }
  return (<Row onUpdate={onUpdate} {...props} onNavigateAudit={onNavigateAudit}/>)
}

export const TaskTable = ({ tasks, onUpdate, renderHeader, sectionId, isHaveMultipleBuilding, onNavigateAudit }) => {
  ...
  return revTask.map((task, index) => {
    return (
      renderRow(onUpdate, onNavigateAudit)({ ...task, index, isHaveMultipleBuilding })
    )
  })
  // <SectionList

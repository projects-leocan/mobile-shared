...
export default class Floor extends Component {
  ...
  render() {
    const { floor, rooms, stats, floorTasks, buildingName } = this.props.data;
      ...
      <Container onPress={() => handlePress(floor.id)}>
        ...
        <PrimaryRow>
          <FloorText isSpecial={isSpecial}>
            { isSpecial ? `${floor.number} ${isAparthotelSettings ? I18n.t('base.ubiquitous.properties') : I18n.t('attendant.main.index.rooms')}`.toUpperCase() 
            : `${buildingName} ${floor.number}`.toUpperCase() }
          </FloorText>
          <TasksText>

...
const RoomRow = ({ room, activateRoom, displayUser, auth, isAttendantApp}) => {
  ...
  const anonymizeGuestName = auth?.config?.anonymizeGuestName;
        ...
        <View style={{ maxWidth: "82%" }}>
          ...
          <View style={[flxRow, aic, jcsb]}>
            <View style={[flxRow, aic, { flexWrap: 'wrap' }]}>
              ...
              {currentGuest ?
                <Text style={styles.guestName}>{` · ${currentGuest.name ? isAttendantApp && anonymizeGuestName ? 'no name' : currentGuest.name : ''} (${occupants}) · ${I18n.t(`base.ubiquitous.${guestStatus}`).toUpperCase()}`}</Text>
                : null
              }
            </View>
            {

...
const RoomRow = ({ room, activateRoom, displayUser, auth, isAttendantApp=false}) => {
  const turndownService = isAttendantApp ? get(room, 'turndownService', '') : get(room, 'roomHousekeeping.turndownService', '');
  const { name, guests, guestStatus, roomCategory, turndownUser, roomHousekeeping: { color = "000" } } = room;

  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        />
        <View style={[flexGrow1, { height: '100%' }]}>
          <ListView
            data={filterRooms}
            ...
            getSectionId={(row) => {
              const buildingName = get(row, 'buildingName')
              const floorName = get(row, 'floorName')
              const validateSectionName = buildingName ? `${buildingName} \u2022 ${floorName}` : floorName;

...
const ModalContent = ({ activeRoom, dismiss, submit, config, onNavigate, specificRoomUpdate, isAllowDoneTurnDown }) => {
  ...
  const  attendantMinimumMinutes = config.attendantMinimumMinutes;
  ...
  return (
    <View style={[styles.container, flx1]}>
      ...
      <ScrollView>
        ...
        {isDone ? turndownService === "cleaning" && allowDone === true ?
          <View>
          ...
          <View>
            ...
            <View style={styles.subBtns}>
              {/* <TouchableOpacity style={[styles.subBtn, margin.r5, { backgroundColor: "#65E5D8" }]} onPress={() => submit('cleaning')}>
                <Text><IcoMoonIcon name={'bed'} size={36} color='white' /></Text>
                <Text style={styles.btnText}>{I18n.t('attendant.clean.menu.start-cleaning').toUpperCase()}</Text>
              </TouchableOpacity> */}

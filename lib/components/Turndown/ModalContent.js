...
const ModalContent = ({ activeRoom, dismiss, submit, config, onNavigate, specificRoomUpdate, isAllowDoneTurnDown,auth, isAttendantApp=false }) => {
  const { name } = activeRoom || {};
  const turndownService = isAttendantApp ? get(activeRoom, 'turndownService', '') : get(activeRoom, 'roomHousekeeping.turndownService', '');

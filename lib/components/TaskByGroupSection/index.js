...
export default class TaskByGroupSection extends React.Component {
    ...
    renderSubGroupItem = (data, isExtraUser = false) => {
        const { renderHeader, isHaveMultipleBuilding } = this.props;
        return (
            <ListView
                data={data}
                renderRow={(row, _, index) => <Item index={index} renderHeader={renderHeader} isHaveMultipleBuilding={isHaveMultipleBuilding} onPress={isExtraUser ? this._handleExtraUserSelect : this._handleUserSelect} row={row} />}
            />
        )
    }

                ...
                renderItem={({ item: inspection, index, section }) => {
                  return (
                    <Inpst
                      ...
                      // onChange={(field, value) => !readOnly && onChange(inspection?.question, field, value, section?.title)}
                      onChange={(answer) => !readOnly && onChange(inspection?.question, answer, section?.title)}
...
class AuditLogic extends React.Component {
  ...
  handleChange = (inspection, answerObj, sectionTitle) => {
    setTimeout(() => {

...
class AuditView extends React.Component {
  ...
  render() {
    ...
    //   return pv;
    // }, [])
    // let auditInsert = JSON.parse(AsyncStorage.getItem("INSERT_AUDIT"));
    // console.log("auditInsert using asyncstorage",auditInsert);
    if (auditInsert !== null) {
      const auditInsertData = filter(auditInsert, function(item){
                      return auditId === item.id ? item.data : null
                    })
      if (auditInsertData != null && auditInsertData.length > 0) {
        inspections = auditInsertData[0].data;
      }
     }
...
class AuditLogic extends React.Component {
  ...
  handleChange = (inspection, answerObj, value, sectionTitle) => {
    console.log("answerObj inside handle change +++++",answerObj);
    setTimeout(() => {
      const inspections = this.state.inspections
      ...
      this.setState(({ inspections }) => {
        const updatedSection = inspections.map((sectionObj, secIndex) => {
          if (secIndex === sectionIndex) {
              ...
              if (get(o, 'question', '') === inspection) {
                if (value) {
                  if (answerObj === "photo") {
                    ...
                    const filterSignature = filter(get(o, 'answerImageUrls', ''), function (ans) {
                      return (
                        ans.key === "note"
                      )
                    })
                    return {
                      ...o,
                      // answerImageUrls: o.answerImageUrls ? value : value
                      answerImageUrls: filterSignature.length !== 0 ? [...filterSignature, ...value] : value
                    }
                  }
                  else {
                    return {
            ...
            // this.props?.addAudit(ObjectToSend)
            // this.props.insertAudit({id:this.props.auditId, data:{ObjectToSend}})
            return ObjectToSend
          } else {
        ...
        // AsyncStorage.setItem("INSERT_AUDIT", JSON.stringify({id:this.props.auditId, data:updatedSection}))
        this.props.insertAudit({id:this.props.auditId, data:updatedSection})
        return ({
          inspections: updatedSection
        })
      })
...
const Audit = ({
 ...
 console.log("auditInsert inside component",auditInsert),
  <>
    <Audit.Logic inspections={inspections} output={output} insertAudit={insertAudit} auditId={auditId}>
      {
        (audit) => {
          return (
            <Audit.View

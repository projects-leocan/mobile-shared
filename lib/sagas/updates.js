...
export default function ({ apiUrl }) {
  ...
  function* updateTurnDownRoomFlow({ field, updateTurnDownStatus, tapTs }) {
    ...
    console.log('--- updateTurnDownRoomFlow ---')
    console.log(roomId)
    console.log(turnDownPayload)
  ...
  function* assignCleaningToUserFlow({ room, user, options }) {
    console.log("assignCleaningToUser", room, user, options);
    const createAssignCleaningObject = {
      hotelId: get(room, 'hotelId', null),
      ...
      // description;,
      credits: null,
      isNightlyCleaning: false,
      isTurnDownCleaning: false
    }

    yield put(OutboundActions.assignNewCleaning(createAssignCleaningObject));
  ...
  function* reAssignCleaningToUserFlow({ room, user, options }) {
    console.log("assignCleaningToUser");
  }
  ...
  function* createTaskFlow({ data }) {
    ...
    if (imageUrl.length > 0) {
      ...
      const uploadedImage = yield call(generateUploadedImageArray, imagePayload);
      const task = { ...data, hotelId: userHotelId, imageUrls: uploadedImage };

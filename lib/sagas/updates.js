...
export default function ({ apiUrl }) {
  ...
  function* updateTurnDownRoomFlow({ field, updateTurnDownStatus, tapTs }) {
    ...
    yield all([put(RoomsActions.updateRoomOptimistic({ roomId, field: 'turndownService', value: status }))])
    yield all([put(OutboundActions.updateNightlyCleaningStatus(roomId, turnDownPayload))])
    // updateNightlyCleaningStatus
  }

  // Update Room
  function* updateRoomFlow({ roomId, status, tapTs, roomName }) {
    ...
    try {
      ...
      if (status === "cleaning") {
        yield all([put(OutboundActions.roomCleanStart(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomCleanStart(roomId, cleanListRoom,roomName));
      } else if (status === "paused") {
        yield all([put(OutboundActions.roomCleanPause(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomCleanPause(roomId, cleanListRoom, roomName));
      } else if (status === "finish") {
        yield all([put(OutboundActions.roomCleanFinish(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomCleanFinish(roomId, cleanListRoom, roomName));
      } else if (status === "delay") {
        yield all([put(OutboundActions.roomDelay(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomDelay(roomId, cleanListRoom, roomName));
      } else if (status === "dnd") {
        yield all([put(OutboundActions.roomDND(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomDND(roomId, cleanListRoom, roomName));
      } else if (status === "refuse") {
        yield all([put(OutboundActions.roomRefuse(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomRefuse(roomId, cleanListRoom, roomName));
      } else if (status === "voucher") {
        yield all([put(OutboundActions.roomVoucher(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomVoucher(roomId, cleanListRoom, roomName));
      } else if (status === "no-check") {
        yield all([put(OutboundActions.roomNoCheck(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomNoCheck(roomId, cleanListRoom, roomName));
      } else if (status === "confirm-dnd") {
        yield all([put(OutboundActions.roomConfirmDND(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomConfirmDND(roomId, cleanListRoom, roomName));
      } else if (status === 'cancel-finish') {
        yield all([put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-finish', roomName))])
        // yield put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-finish', roomName));
      } else if (status === 'cancel-dnd') {
        yield all([put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-dnd', roomName))])
        // yield put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-dnd', roomName));
      } else if (status === 'cancel-refuse') {
        yield all([put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-refuse',roomName))])
        // yield put(OutboundActions.roomCancelStatus(roomId, cleanListRoom, 'cancel-refuse', roomName));
      } else if (status === "") {
        yield all([put(OutboundActions.roomCancel(roomId, cleanListRoom,roomName))])
        // yield put(OutboundActions.roomCancel(roomId, cleanListRoom, roomName));
      }
  ...
  function* updateNightyInspectionStatus({ field, updateTurnDownStatus, tapTs }) {
    ...
    yield all([put(OutboundActions.updateNightlyInspectionStatusFu(roomId, turnDownPayload))])
  ...
  function* watchUpdateRoomVoucher(state) {
    ...
    yield takeEvery(UpdatesTypes.ROOM_VOUCHER, updateRoomFlow);
  }
  ...
  function* resetRoomFlow({ roomId, turnDown }) {
    ...
    try {
      ...
      if (turnDown) {
        ...
        yield all([put(OutboundActions.roomHousekeepingUpdate(roomIdNew, newStatus, objectToSend.cleaningId))])
  ...
  // Room Model
  function* updateRoomModelFlow({ room, status, codeName }) {
    const roomId = get(room, 'id', null);
    const roomName = get(room, 'name', null)
    const loginUser = yield select(userSelector);
    const hotelPlannings = yield select(planningsSelector);
    const cleanListRoom = find(hotelPlannings, { room_id: roomId }) || {};

    yield put(RoomsActions.isSocketFireOnUpdateInspection({ isFire: false }))
    yield put(RoomsActions.addSocketList(roomName, status, _.isEmpty(codeName) ? "" : "Status: " + codeName , "Update inspection status", "/Cleaning/UpdateInspectionStatus" ))
    if (isEmpty(cleanListRoom, true)) {
      try {
        ...
        yield all([put(OutboundActions.roomHousekeepingUpdate(roomId, status, customeCleanPayload, roomName, codeName))])

        // yield put(OutboundActions.roomHousekeepingUpdate(roomId, status, customeCleanPayload, roomName, codeName));
      } catch (error) {
        console.log(error)
      }
    } else {
      ...
      try {
        ...
        yield all([put(OutboundActions.roomHousekeepingUpdate(roomId, status, cleanListRoom, roomName, codeName))])
        return true;
      } catch (error) {
        console.log(error);
      }
  ...
  function* watchRoomModelHousekeeping() {
    ...
    yield takeEvery(UpdatesTypes.ROOM_HOUSEKEEPING, updateRoomModelFlow);
  }

  function* watchRoomModelUpdateNightyInspection() {
    yield takeEvery(UpdatesTypes.ROOM_NIGHTY_HOUSEKEEPING, updateNightyInspectionStatus);
  ...
  function* watchRoomModelTurndown() {
    ...
    yield takeEvery(UpdatesTypes.ROOM_TURNDOWN, updateTurnDownRoomFlow);
  ...
  function* submitLostItemFlow(insertLFPayload) {
    try {
      ...
      yield all([put(OutboundActions.submitLostItem(insertLFPayload))]);
      yield all([put(UpdatesActions.lostItemAddSuccess(null))]);
  ...
  function* roomScheduleGroupedAssetConsumationFlow({ groupedAssetConsumation }) {
    try {
      yield put(OutboundActions.inventoryScheduleGroupedAssetConsumption(groupedAssetConsumation));
    } catch (error) {
      console.log(error);
    }
  }

  function* watchRoomScheduleGroupedAssetConsumationFlow() {
    yield throttle(3000, UpdatesTypes.INVENTORY_GROUPED_ASSET_CONSUMATION, roomScheduleGroupedAssetConsumationFlow);
  }
  ...
  // Update Task  
  function* updateTaskFlow({ uuid, status, auditId, auditTemplateId, isAuditRequired}) {
    const userId = yield select(userIdSelector);
    ...
    try {
      ...
      yield all([put(RoomsActions.taskUpdateOptimistic({ uuid, update: optimistic }))])
      yield all([put(OutboundActions.taskUpdate(uuid, data, auditId, auditTemplateId, isAuditRequired))])
    } catch (error) {
      console.log(error)
    }
  }
  ...
  function* createTaskMessage({ taskId, message }) {
    try {
      ...
      yield all([put(OutboundActions.taskCreateMessage(payload))])
    } catch (error) {
      console.log(error)
    }
  }
  ...
  function* watchUpdateCreateTaskMessageFlow(state) {
    ...
    yield takeEvery(UpdatesTypes.CREATE_TASK_MESSAGE, createTaskMessage);
  ...
  function* reassignTaskFlow({ uuid, user, dueDate }) {
    ...
    try {
      ...
      yield all([put(OutboundActions.taskReassign(uuid, data))])
    } catch (error) {
      console.log(error);
    }
  }
  ...
  function* departureTaskFlow({ uuid }) {
    try {
      ...
      yield all([put(OutboundActions.taskDeparture(uuid, data))])
    } catch (error) {
      console.log(error);
    }
  }
  ...
  function* watchDepartureTaskFlow(state) {
    ...
    yield takeEvery(UpdatesTypes.TASK_DEPARTURE, departureTaskFlow);
  ...
  function* taskAdditionalPhotoFlow({ taskId, imageUrls }) {
    ...
    try {
      if (imageUrls.length > 0) {
        ...
        yield all([put(OutboundActions.uploadTaskAdditionalPhoto(additionalImagePayload))])
      } else {
        const additionalImagePayload = {
          hotelId: userHotelId,
          taskId: taskId,
  ...
  function* watchTaskAdditionalPhotoFlow() {
    // yield throttle(3000, UpdatesTypes.TASK_ADDITIONAL_PHOTO, taskAdditionalPhotoFlow);   
     yield takeEvery(UpdatesTypes.TASK_ADDITIONAL_PHOTO, taskAdditionalPhotoFlow);
  ...
  const watchers = {
    ...
    watchRoomScheduleGroupedAssetConsumationFlow,
  ...
  return {
    ...
    sagas: {
      ...
      roomScheduleGroupedAssetConsumationFlow,

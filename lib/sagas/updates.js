...
import { userIdSelector, userSelector, hotelIdSelector } from '../selectors/auth';
...
export default function ({ apiUrl }) {
  ...
  function* roomUpdateRefuse(roomId, cleanListRoom, headerObj) {
    const { id: cleaningId } = cleanListRoom;
    // const userHotelId = yield select(hotelIdSelector);
    return yield call(authRequest, ROOM_UPDATE_API, {
      method: 'POST',
      body: JSON.stringify({
        attendantStatus: 'refuse',
        tapTs: '0',
        cleaningId
      })
    });
  ...
  function* updateRoomFlow({ roomId, status, tapTs }) {
    ...
    const hotel_group_key = yield select(hotelNameSelector);

    console.log('---- updateRoomFlow token -----');
    console.log(token)
    console.log(hotel_group_key)
    ...
    try {
      ...
      const cleaningList = yield call(fetchCleaningList);
      const cleanListRoom = find(cleaningList, { room_id: roomId }) || {};

      console.log('---- cleaningList ----');
      console.log(cleaningList);
      console.log(cleanListRoom);
      console.log(isEmpty(cleanListRoom, true));
      ...
      if (status === "cleaning") {
        ...
        // yield put(OutboundActions.roomRefuse(roomId, cleanListRoom, headerObj));
        const data = yield call(roomUpdateRefuse(roomId, cleanListRoom, headerObj));
        console.log('---- roomRefuse ----');
        console.log(data)

...
export default function ({ apiUrl }) {
  ...
  function* updateRoomModelFlow({ room, status }) {
    ...
    if (isEmpty(cleanListRoom, true)) {
      // const response = yield call(() => roomPlanningFlow({ room, user: loginUser }));
      const response = yield call(() => createPlanning({ room, user: loginUser }));
      if (get(response, 'isSuccess', false)) {
        const cleannerId = get(response, 'data', null)

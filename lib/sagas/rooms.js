...
export default function ({ apiUrl, userType }) {
  ...
  function* fetchRoomsFlow(socketData) {
    try {
      ...
      yield put(RoomsActions.roomsFailure({status: 200, message:""}));
      ...
      let err = {
        status: e.status,
        message: (e.status === "Request timeout" || e.status === "Network request failed") ? "Looks like your internet is slow... Please try to relogin" : "Something went wrong!!"
      }
      yield put(RoomsActions.fetchRoomLoader(false))

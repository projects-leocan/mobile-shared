...
import UpdateAction from '../actions/updates'
...
export default function ({ apiUrl, userType }) {
  ...
  function* fetchAuditTasksFlow(taskId) {
    try {
      const data = yield call(() => fetchTasks(taskId.id));
      yield put(RoomsActions.auditTasksSuccess(data));
      yield put(UpdateAction.taskRemoveAuditMeta())
    } catch (e) {
      console.log("tasks Failure", e);
      let err = {
        status: e.status,
  ...
  function* watchSubmittedAuditTasksFlow() {
    yield throttle(3000, RoomsTypes.AUDIT_TASKS_FETCH, fetchAuditTasksFlow);
  }

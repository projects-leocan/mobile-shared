...
import { planningsForUser, attendantRooms, planningsSelector, roomsSelector, hotelMessageSelector, nightyPlanningsForUser } from '../selectors/rooms';
...
export default function ({ apiUrl, userType }) {
  ...
  function* fetchTasksFlow(taskId) {
    try {
      ...
      const { rooms: { hotelTasks } } = yield select();

      console.log("yield select()", yield select());

      if (!_.isEmpty(getSpecificTaskId)) {
        const { hotelsTask: { activeAvailHotel } } = yield select();
        const getSpecificTask = get(taskId, 'data', [])
        const filterTask = getSpecificTask.filter((d) => !hotelTasks.some(h => h.id === d.id))
        yield put(RoomsActions.setTaskSocketData(filterTask));
      }
      const data = yield call(() => fetchTasks(getSpecificTaskId));
      let newData = []
      if (_.isEmpty(getSpecificTaskId)) {
        ...
        console.log("hotelTasks ======== ", hotelTasks);

        // const newDataMap = _.keyBy(addDueDate, 'id');


        // console.log("addDueDate", addDueDate);
        // console.log("newDataMap =====", newDataMap);
        // console.log("newData ======== ", hotelTasks);

        // newData = hotelTasks.map(item => newDataMap[item.id] ? newDataMap[item.id] : item);
        // const hotelTaskMap = _.keyBy(hotelTasks, 'id');

        // console.log("hotelTaskMap =====" , hotelTaskMap);

        // // const getSpecificTask = get(taskId, 'data', [])
        // // const filterTask = getSpecificTask.filter((d) => !hotelTasks.every(h => h.id === d.id))

        // // if (!_.isEmpty(filterTask)) {
        // //   newData.push(...filterTask)
        // // }

        // // console.log("filterTask ======", filterTask);

        // addDueDate.forEach((item) => {
        //   console.log("newDataMap[item]", !hotelTaskMap[item.id]);
        //     return (
        //       !hotelTaskMap[item.id] && newData.push(item)
        //     )
        // });

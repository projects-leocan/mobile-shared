...
import RoomTypes from '../constants/rooms';
...
export default function ({ apiUrl, userType }) {
  ...
  function* updateFirebaseDeviceKey() {
    ...
    const response =  yield call(authRequest, UPDATE_FIREBASE_DEVICE_KEY, {
      method: 'POST',
      body: JSON.stringify({
    ...
    console.log("------ API updateFirebaseDeviceKey -----", JSON.stringify(response))
    Sentry.captureMessage("API updateFirebaseDeviceKey");
    return response;
  }
  ...
  function* removeFirebaseDeviceKey() {
    ...
    console.log("------ API removeFirebaseDeviceKey -----", JSON.stringify(response))
    Sentry.captureMessage("API removeFirebaseDeviceKey");
    return response;
  }
  // USER VERSION

...
export default function ({ apiUrl, userType }) {
  ...
  function* removeFCMTokenFlow() {
    try {
      const data = yield call(removeFirebaseDeviceKey);
      yield put(AuthActions.removeFCMTokenSucess(data));
    } catch (e) {
      console.log(e);
    }
  }

  function* watchRemoveFCMTokenFlow() {
    yield takeLatest(AuthTypes.REMOVE_FCM_KEY_REQUEST, removeFCMTokenFlow)
  }

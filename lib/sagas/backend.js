import { takeLatest, put, call, select, take, throttle } from 'redux-saga/effects';
...
export default function({ apiUrl, userType }) {
  ...
  function * onlineFlow({ isOnline }) {
    ...
    // yield put(BackendActions.futurePlanningsBackend())
  
    // yield put(BackendActions.roomStatusesBackend());
    // yield put(BackendActions.roomHousekeepingsBackend());
  ...
  function * backendFuturePlanningsFlow() {
    const { auth: { hotelId, token }, backend: { plannings } } = yield select();
    const currentTs = moment().unix();

    try {
      if (!plannings.lastUpdate || currentTs - plannings.lastUpdate > STALE_TIME) {
        yield put(RoomsActions.futurePlanningsFetch());
      }
    } catch (e) {
      console.log(e);
    } finally {
  ...
  function * watchBackendFuturePlanningsFlow() {
    yield takeLatest(BackendTypes.FUTURE_PLANNINGS_BACKEND, backendFuturePlanningsFlow);
  }
  ...
  const backendRoomMessagesFlow = backendFactory('roomMessages', RoomsActions.roomMessagesFetch, LONG_STALE_TIME);
  function * watchBackendRoomMessagesFlow() {
    yield takeLatest(BackendTypes.ROOM_MESSAGES_BACKEND, backendRoomMessagesFlow)
  }

  const backendAvailHotelsTaskFlow = backendFactory('availHotelsTask', HotelsTaskActions.availHotelsTaskFetch, LONG_STALE_TIME);
  function * watchBackendAvailHotelsTaskFlow() {
    yield takeLatest(BackendTypes.AVAIL_HOTELS_TASK_BACKEND, backendAvailHotelsTaskFlow)
  }

  const backendAvailHotelsRoomFlow = backendFactory('availHotelsRoom', HotelsTaskActions.availHotelsRoomFetch, LONG_STALE_TIME);
  function * watchBackendAvailHotelsRoomFlow() {
    yield takeLatest(BackendTypes.AVAIL_HOTELS_ROOM_BACKEND, backendAvailHotelsRoomFlow)
  }

  const backendRoomInspectionFlow = backendFactory('roomInspectionList', RoomsActions.roomInspectionCleanFetch, LONG_STALE_TIME);
  function * watchBackendRoomInspectionFlow() {
    yield takeLatest(BackendTypes.ROOM_INSPECTION_CLEAN_BACKEND, backendRoomInspectionFlow)
  }
  ...
  const watchers = {
    ...
    watchBackendFuturePlanningsFlow,
    ...
    watchBackendRoomMessagesFlow,
    watchBackendAvailHotelsTaskFlow,
    watchBackendAvailHotelsRoomFlow,
    watchBackendRoomInspectionFlow
  };

...
import { ActivityIndicator, FlatList, InteractionManager, Platform, SafeAreaView } from 'react-native';
...
import { generateThumbnailFromVideo, validateFileType } from 'rc-mobile-base/lib/utils/file-type';
...
import { assignedTasks } from 'rc-mobile-base/lib/layouts/Tasks/selectors';
...
import { platform } from 'rc-mobile-base/lib/utils';
import Permissions from 'rc-mobile-base/lib/utils/Permissions';
import { useState } from 'react';
import { logClean } from 'rc-mobile-base/lib/actions/outbound';
...
class TaskDetail1 extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        // })
        this.generateThumbnailFromVideoMain();
    ...
    renderRoomGuestInfo = () => {
        ...
        const isAttendantApp =this.props.screenProps.isAttendantApp;
        const anonymizeGuestNames = this.props.config?.anonymizeGuestNames;
        ...
        return (
            <HorizontalContainer>
                ...
                {isGuestReq ? <ItemLabelDesc style={{ flex: 0.6 }}>{guestName ? isAttendantApp && anonymizeGuestNames ? 'no name' : guestName : ''} {checkIn} {checkOut}</ItemLabelDesc> : null}
            </HorizontalContainer>
        )
    }
    ...
    generateThumbnailFromVideoMain = async () => {
        let getTask = get(this.props, "task", [])
        const taskImageURLs = compact(get(getTask, 'image_urls', []))
        const taskVideoURLs = compact(get(getTask, 'video_urls', []))

        const mapTaskImageUrls = taskImageURLs.map((image) => {
            return {
                type: 'image',
                url: image
            }
        })
        // this.setState({task: {...getTask,mergeObject:[...mapTaskImageUrls, ...taskVideoURLs]}})

        let mapTaskVideoUrls = taskVideoURLs.map(async (image) => {
            return {
                type: 'Video',
                url: image,
                path: await generateThumbnailFromVideo(image)
            }
        })

        mapTaskVideoUrls = await Promise.all(mapTaskVideoUrls)
        const finalArray = [...mapTaskImageUrls, ...mapTaskVideoUrls]
        this.setState({ task: { ...getTask, mergeObject: finalArray } })
    ...
    renderTaskImages = (item, index) => {
        if (item?.type) {
            if (item?.type === "Video") {
                // let video = await generateThumbnailFromVideo(image)
                return (
                    <TaskImageCellContainer activeOpacity={0.7} onPress={() => this.onPlayVideo(item.url)}>
                        <TaskImageView source={{ uri: item?.path }} ></TaskImageView>
                        <VideoPlayContainer>
                            <VideoPlayTouchable onPress={() => this.onPlayVideo(item.url)}>
            ...
            return (
                ...
                <TaskImageCellContainer activeOpacity={0.7}
                    style={{
                        justifyContent: "center"
                    }}
                >
                    <ActivityIndicator size="small" color={splashBg.color} />
                </TaskImageCellContainer>
            )
        }
    ...
    _updateTask = (status, isTask) => {
        ...
        if (validateStatus && validateStatus === "convert") {
            ...
            if (is_audit_required && is_audit_answered === false && validateStatus === 'completed') {
                ...
                if (!_.isEmpty(auditData)) {
                  ...
                  this.props.navigation.navigate('AuditEdit', { audit: finalData, onGoBack: () => this.fetchAuditTask(uuid) })
                  
                }else{
                  return true
                }
              }else if(is_audit_required && is_audit_answered && validateStatus === 'completed'){
                this.props.updateTask(uuid, validateStatus, audit_id, audit_template_id, is_audit_required)
                this.props.navigation.goBack()
              }else{
                this.props.updateTask(uuid, validateStatus, audit_id, audit_template_id, is_audit_required)
              }
    ...
    onAddTaskImage = () => {
        ...
        if (this.SelectedAssets) {
            ...
            const updatedAsset = assetsHolder && assetsHolder.map((data) =>{
                if (data.assetsType === "video") {
                    return({
                    type: 'Video',
                    url: data.assetsPath,
                    path: data.path
            ...
            const finalArray = [...this.state.task.mergeObject, ...updatedAsset]
            this.setState({ task: {mergeObject: finalArray } })
            this.setState({ isAddMoreImage: false })
        }
    }


    onTakeImages = async (data) => {
    ...
    render() {
        ...
        const { isShowTaskImageModal, initialTaskImageIndex, isTimePickerVisible, selectedTime, isShowVideo,
            videoSource, taskMessage, isAddMoreImage, isCaptureImage } = this.state;
        ...
        const guest_name = get(task, 'guest_info.guest_name', null)
        const isAttendantApp =this.props.screenProps.isAttendantApp;
        const anonymizeGuestNames = this.props.config?.anonymizeGuestNames;

        let taskMessages = [];
        taskMessages = orderBy(get(task, 'fullMessages', []), ['date_ts'], ['asc']);
        let videoAndImageDispaly = get(this.state.task, 'mergeObject', [])
        let imageData = []
        videoAndImageDispaly.filter((data) => {
            if (data.type === 'image') {
               imageData.push(data.url)
            }
        })
        if (comment) {
            const commentObj = {
                        ...
                        >
                            ...
                            `base.ubiquitous.${task.category}-task`
                                ...
                                {(get(task, 'category', '') == 'priority' || isGuestReq) ?
                                    <>
                                        <SubheaderStatusTextCopy>{task?.category ? this.capitalize(task?.category) : null} task</SubheaderStatusTextCopy>
                                        <Icon
                                            name="star"
                                    ...
                                    :
                                    <SubheaderStatusText>{task?.category ? this.capitalize(task?.category) : null} task</SubheaderStatusText>
                                }
                                {get(task, 'created_at_string', '') ? <SubheaderTimeText>{moment(task?.created_at_string).fromNow()}</SubheaderTimeText> : null}
                            </SubheaderContainer>
                        ...
                        {isArray(validateAction)
                            ...
                            : <Item label={I18n.t("base.ubiquitous.action")} value={validateAction} />} */}
                                ...
                                <TaskImageViewer
                                    ...
                                    images={map(imageData, function (obj) { return { url: obj } })}
                                ...
                                <ModalImageContainer isVisible={isAddMoreImage}>
                                    <ModalImageWrapper>
                                        ...
                                        <SelectedAssets
                                            ...
                                            onTakeImages={this.onTakeImages}
                                            isCaptureImage={isCaptureImage}
                                        />
                                    </ModalImageWrapper>
                                </ModalImageContainer>

                                {!isEmpty(videoAndImageDispaly)
                                    ?
                                    <FlatList
                                        data={videoAndImageDispaly}
                                        horizontal
                            ...
                            <InformationContainer>
                                ...
                                {guest_name ? <Item label={`${I18n.t("base.popup.index.guest-name")}`} value={guest_name ? isAttendantApp && anonymizeGuestNames ? 'no name' : guest_name : ''} /> : null}
                            ...
                            <InformationContainer>
                                <CellHeader label={I18n.t("maintenance.task.index.created-by")} />
                                <ItemContainer>
                                    <ItemLabelDesc>{task?.creator?.fullName}</ItemLabelDesc>
                                </ItemContainer>
                            </InformationContainer>
...
const mapStateToProps = (state, props) => {
    ...
    const task = activeTab === 2 ? userSentTask(props.navigation.state.params.task.uuid)(state) : currentTask(props.navigation.state.params.task.uuid, activeTab)(state);
    ...
    return {
        ...
        assignmentOptions: assignmentOptionsSelector(props?.screenProps)(state),
...
const TaskDetail = (props) => {
    ...
    // const generateThumbnailFromVideoMain = async () => {
    //     let getTask = get(props, "task", [])
    //     const taskImageURLs = compact(get(getTask, 'image_urls', []))
    //     const taskVideoURLs = compact(get(getTask, 'video_urls', []))
    //     const mapTaskImageUrls = taskImageURLs.map((image) => {
    //         return {
    ...
    //         }
    //     })
    //     setTask({ ...getTask, mergeObject: [...mapTaskImageUrls, ...taskVideoURLs] })
    ...
    //     mapTaskVideoUrls = await Promise.all(mapTaskVideoUrls)
    //     const finalArray = [...mapTaskImageUrls, ...mapTaskVideoUrls]
    //     setTask({ ...getTask, mergeObject: finalArray })
    // }

    // useEffect(() => {
    //     generateThumbnailFromVideoMain()
    // }, [])
    return <TaskDetail1 insets={insets} {...props} />
}

...
import Header from "rc-mobile-base/lib/components/AssignmentOptions/Header"

import { extend } from 'lodash/object';
import Row from "rc-mobile-base/lib/components/AssignmentOptions/Row";
...
export default class SelectBoth extends React.Component {
  ...
  _handleSelectOption = (option, data) => {
    const { selectedOptions, selectedObject,user } = this.state;
    ...
    if (selectedObject.value === data.value) {
      this.setState({ selectedObject: {} })
    }
    else {
      this.setState({ selectedObject: data,user:data })
      this.tabView.current.setPage(1)
    }
  }
  ...
  render() {
    ...
    const { searchQuery, user, date, today, activeGuestDate, selectedOptions } = this.state;
    ...
    const newUserSections = userSections.map(o => extend({}, o, { isSelected: selectedOptions.includes(o.value) }));
    const filteredUserSections= cleanQuery ? newUserSections.filter(o => o.name.toLowerCase().includes(cleanQuery)) : newUserSections

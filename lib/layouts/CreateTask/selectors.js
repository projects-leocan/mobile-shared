...
const getSelectedLocation = state => state.filters.roomLocation;
...
export const locationsSelectorData = createSelector(
  [allLocationsSelector, roomSearchForm],
  (locations, roomSearch) => {
    if (!roomSearch) {
      // return locations
      return locations.map((d) => {
        return {
          id: d.id,
          name: d.name,
          isTemporaryRoom: d.isTemporaryRoom,
    ...
    const regex = new RegExp(roomSearch.search, 'i')
    const Data = locations.filter(location => location.name.match(regex))
...
export const getSelectedLocationFilter = createSelector(
  [getSelectedLocation, sortedRooms],

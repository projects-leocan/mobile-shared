...
import { map, filter, omit, flatten, uniqBy, first } from 'lodash';
...
class AssignmentSelect extends Component {
  ...
  onNextPage = () => {
    ...
    const finalAssignee = [...userGroupIds, ...userSubGroupIds, ...userIds]
        ...
        <BodyContainer>
          ...
          <ListContainer>
            <ListView
              ...
              renderSectionHeader={(label) => <GroupHeader label={label} />}
              renderFooter={() => <ListFooter />}
              getSectionId={(data) => data.name}
            />
          </ListContainer>

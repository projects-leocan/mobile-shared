...
import { map, filter, omit, flatten, uniqBy, first, compact } from 'lodash';
...
class AssignmentSelect extends Component {
  ...
  _handleGroupSelect = (value) => {
    const { assigneeHolder } = this.state;

    const filterGroup = map(assigneeHolder, function (obj) {
    ...
    this.setState({ assigneeHolder: [...filterGroup] })
  }

  _handleSubGroupSelect = (value) => {
    const { assigneeHolder } = this.state;

    const filterSubGroup = map(assigneeHolder, function (obj) {
    ...
    this.setState({ assigneeHolder: [...filterSubGroup] })
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    const { assignmentOptions } = this.props;
    const { assignmentOptions: prevAssignmentOptions } = prevProps;

    let options = [...assignmentOptions];

    if (assignmentOptions !== prevAssignmentOptions) {
      ...
      this.setState({ assigneeHolder: validateOptions })
    }
  }
  ...
  onNextPage = () => {
    ...
    const mapSelectedSubGroup = uniqBy(flatten(map(assigneeHolder, 'userSubGroup')), 'value');
    ...
    const mapSoloAssignee = uniqBy(flatten(map(assigneeHolder, 'groupUsers')), 'value');
    ...
    const mapAllAssigneeForGroup = map(assigneeHolder, 'groupUsers');

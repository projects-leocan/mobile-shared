...
import { splashBg, white, themeTomato, isTab } from 'rc-mobile-base/lib/styles';
...
class TaskOptions extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const isReRender = nextProps.isAttendantApp ? nextProps.isEmptyFinalAssignee ? 3 : 2 : 3
  ...
  _renderGuestReq() {
    const { isGuestReq, handleUpdateGuestReq } = this.props;

    return (
      <View style={styles.itemContainer}>
        <SectionHeading style={{ paddingLeft: 15, marginTop: 15, flex: isTab ? 3 : 1, }}>{I18n.t(`base.ubiquitous.task-guest-request`)}</SectionHeading>
        <View style={styles.optionContainer}>
          <TouchableOpacity style={[styles.optionBtn, isGuestReq == null ? null : isGuestReq ? { backgroundColor: activeGreen } : null]} onPress={() => handleUpdateGuestReq(true)}>
            <Text style={styles.bigBtnText}>{I18n.t(`base.ubiquitous.yes`)}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.optionBtn, isGuestReq == null ? null : isGuestReq ? null : { backgroundColor: activeGreen }]} onPress={() => handleUpdateGuestReq(false)}>
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <TaskButton
          label={I18n.t('base.ubiquitous.task-submit')}

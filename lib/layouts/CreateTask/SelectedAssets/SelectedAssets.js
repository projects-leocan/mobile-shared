...
import { View, TouchableOpacity, FlatList, Image, Dimensions, SafeAreaView, Alert, Platform, ActivityIndicator } from 'react-native';
...
import { themeTomato, white, splashBg } from 'rc-mobile-base/lib/styles';
import {
...
export default class SelectedAssets extends React.Component {
    ...
    onTakeGallery = async (assetsType) => {
        try {
            ...
            let result
            let assetsPayload = await Promise.all(ImageResponse.filter((item) => {
                return (
                ...
                try {
                    if (assetsType === "video") {
                        result = await Video.compress(
                            ...
                            (progress) => {
                                this.setState({isCompressdone: true})
                            }
                        );
                        this.setState({isCompressdone: false})
                    }
                    else if (assetsType === "photo") {
    ...
    _handleTakePicture = async () => {
        ...
        await this.camera.takePictureAsync(options).then(async data => {
            ...
            const statResult = await stat(photoPath);
            let result
            if (photoPath) {
                try{
                    result = await ImageCompress.compress(photoPath, {
                        compressionMethod: 'manual',
                        maxWidth: 1000,
                        quality: 0.8,
                      });
    ...
    onTakeImageFromCamera = async (data) => {
        const { onTakeImages } = this.props;
        const value = await RcPermissions.requestPermissions('camera');
    ...
    render() {
                    ...
                    <Modal isVisible={isCaptureImage} style={{ flexGrow: 1, borderRadius: 12, margin: 0 }}>
                        ...
                        <View style={{ flex: 1, justifyContent: "space-around" }}>
                            <ExtraBackgroundContainer>
                                ...
                                <ExtraExitContainer onPress={() => this.onTakeImageFromCamera(false)} >
        ...
        if (get(assetsHolder, 'length') > 0) {
            return (
                <View style={styles.rootContainer}>
                    <View style={styles.topContainer}>
                        ...
                        />
                        {this.state.isCompressdone === true ?
                            <ActivityIndicator size="large" color={splashBg.color} /> : <></>
                        }
                    </View>
                        ...
                        <View style={styles.buttonContainer}>
                            <View style={styles.assetsActionButtonWrapper}>
                                <TaskButton
                                    ...
                                    onPress={() => this.onTakeImageFromCamera(true)} />
                            </View>
            ...
            return (
                <View style={styles.rootContainer}>
                    <View style={{ flex: 1}}>
                        <TouchableOpacity style={styles.imagePlaceHolder} activeOpacity={0.7} onPress={() => this.onTakeImageFromCamera(true)}>
                            {this.state.isCompressdone === true ?
                                <ActivityIndicator size="large" color={splashBg.color} /> : <></>
                            }

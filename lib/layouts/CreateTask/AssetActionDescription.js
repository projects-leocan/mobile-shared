...
class AssetActionDescription extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!prevSelectedAsset && selectedAsset) {
      const existingAssetTasks = roomTasks
        .map(
          task =>
            get(task, "meta.durable_asset_id") ||
            get(task, "meta.virtual_asset_id") ||
            get(task, "meta.asset_id")
        )
        .filter(Boolean);
  ...
  _renderModels() {
    ...
    if (
      !selectedAsset ||
      !selectedAsset.models ||
      !selectedAsset.models.length
    ) {
      return null;
    }

    const models = selectedAsset.models.map(model =>
      extend({}, model, {
        isSelected: selectedModel && model._id === selectedModel._id,
      })
    );
  ...
  _renderSublocations() {
    ...
    const roomId =
      (room && get(room, "_id")) ||
      get(selectedLocations, ["locations", 0, "_id"]) ||
      null;
    if (!roomId) {
      return null;
    }

    const availableSublocations = filter(sublocations, {
      model_id: selectedModel._id,
      room_id: roomId,
    }).map(sublocation =>
      extend({}, sublocation, {
        isSelected:
          selectedSublocation && selectedSublocation.id === sublocation.id,
      })
    );
    ...
    const buttons = availableSublocations.map(sublocation => {
      return (
        <TouchableOpacity
          ...
          style={[
            styles.actionBtn,
            sublocation.isSelected ? styles.selectedActionbtn : null,
          ]}
          onPress={() => handleSelectSublocation(sublocation)}
        >
  ...
  _renderSelectAction() {
    ...
    const buttons = availableOptions.map(action => {
      return (
        <TouchableOpacity
          ...
          style={[
            styles.actionBtn,
            action.isSelected ? styles.selectedActionbtn : null,
          ]}
    ...
    if (createdAsset) {
      rendered = (
        <Text style={styles.actionError}>
          {I18n.t(
            "maintenance.createtask.assetactiondescription.no-actions-for-created-asset"
          )}
        </Text>
      );
      ...
      rendered = (
        <Text style={styles.actionError}>
          {I18n.t(
            "maintenance.createtask.assetactiondescription.you-must-first-select-an-asset"
          )}
        </Text>
      );
      ...
      rendered = (
        <Text style={styles.actionError}>
          {I18n.t(
            "maintenance.createtask.assetactiondescription.there-are-no-actions-for-your-asset"
          )}
        </Text>
      );
    } else {
    ...
    return (
      <View>
        <SectionHeader
          value={I18n.t("base.components.assetactiondescription.action")}
        />
        <View style={styles.actionContainer}>
          {rendered}
        </View>
  ...
  _renderDesc() {
    ...
    return (
      <View>
        <SectionHeader
          value={I18n.t("base.components.assetactiondescription.description")}
        />
        <TextInput
          style={styles.textArea}
          onChangeText={handleUpdateDesc}
          onFocus={() =>
            this.scroll && this.scroll.props.scrollToPosition(0, 160)}
          value={desc}
          multiline={true}
          placeholder={I18n.t(
            "base.components.assetactiondescription.description-placeholder"
          )}
        />
      </View>
    );
  ...
  _renderPriority() {
    ...
    return (
      <View>
        <SectionHeader
          value={I18n.t("base.components.assetactiondescription.priority")}
        />
        <View style={styles.priorityContainer}>
          <TouchableOpacity
            style={[
              styles.priorityBtn,
              isPriority ? { backgroundColor: "#3CC86B" } : null,
            ]}
            onPress={() => handleUpdatePriority(true)}
          >
            <Text style={styles.bigBtnText}>
              {I18n.t("maintenance.createtask.assetactiondescription.yes")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              styles.priorityBtn,
              isPriority ? null : { backgroundColor: "#C93C46" },
            ]}
            onPress={() => handleUpdatePriority(false)}
          >
            <Text style={styles.bigBtnText}>
              {I18n.t("maintenance.createtask.assetactiondescription.no")}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
        ...
        {handleSubmit
          ? <Button
            ...
            >
              <Text style={styles.bigBtnText}>
                {I18n.t(
                  `maintenance.createtask.assetactiondescription.submit-${label ||
                    "maintenance"}`
                ).toUpperCase()}
              </Text>
            </Button>

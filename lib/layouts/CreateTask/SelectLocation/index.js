...
import DateTimeSelect from 'rc-mobile-base/lib/layouts/CreateTask/DateTimeSelect/DateTimeSelect';
...
export default class SelectLocation extends React.Component {
    ...
    if (locations && locations.length) {
      return (
        <View style={styles.sectionContainer}>
          ...
          <TouchableOpacity style={[styles.sectionButton, { position: "relative" }]} onPress={() => this.setState({ isDateModal: !this.state.isDateModal })}>
            <View style={styles.selectionPlaceholderContainer}>
  ...
  _renderTime() {
    ...
    return (
      <View style={styles.sectionContainer}>
        ...
        <TouchableOpacity style={[styles.sectionButton]} onPress={() => this.setState({ isTimeModal: true })}>
          <View style={styles.selectionPlaceholderContainer}>
            <Text style={styles.selectionPlaceholder}>{moment(selectedTime).format("HH:mm:ss")}</Text>
            {/* <RNDateTimePicker value={new Date()} mode="time" style={{ width: 300, opacity: 1 }} display="compact" /> */}
          </View>
          <View style={styles.sectionButtonRightContainer}>
            <MaterialIcons name='chevron-right' size={26} color={splashBg.color} />
          </View>
  ...
  handleTImeCOnfirm = (time) => {
    ...
    this.props.getSelectedDateTime('time', time)

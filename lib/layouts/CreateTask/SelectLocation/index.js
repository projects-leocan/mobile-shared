...
export default class SelectLocation extends React.Component {
  ...
  _handleSelectedAssets(asset) {
    ...
    const { tasks, getSelectedLocation, taskDepartment} = this.props
    const taskList = []
    const rooms = []
    !_.isEmpty(getSelectedLocation) && getSelectedLocation.map((room) =>{
      if (room.isSelected) {
        rooms.push(room)
      }
    })

    !_.isEmpty(tasks) && tasks.map((task) =>{
      if (this.props.isInspector) {
        taskList.push(...task.taskList)
      }else if(this.props.isMaintenance){
    ...
    let matchedRoomName = []
    !_.isEmpty(taskList) && taskList.map((task) => {

      let getName = task.task
      if (_.includes(_.toLower(getName), 'x')) {
        const parts = _.split(getName, 'x');
        if (parts.length >= 2) {
          let getAsset = parts[1]
          let trimAsset = !_.isEmpty(getAsset) ? _.trim(getAsset) : '';
          if(asset.name === trimAsset){
             !_.isEmpty(rooms) && rooms.map((room) =>{
                if (room.id === task?.meta?.room_id && asset.isSelected === false) {
                  matchedRoomName.push(room?.name)
                }
             })
          }else{
            // this.setState({isAssetWarning: false})
          }
        }
      }
    })
    ...
    if (!_.isEmpty(matchedRoomName) && matchedRoomName.length > 0) {
      let uniqRoom = _.uniq(matchedRoomName)
      this.setState({isAssetWarning: true})
      this.setState({warningMessage: "At least one task with this asset exists for the room " + uniqRoom.join(", ")})
    }else{
      if (this.state.isAssetWarning === true) {
        this.setState({isAssetWarning: false})
      }
    }
      ...
      <View style={styles.sectionContainer}>
        ...
        <View style={styles.textAreaContainer} >
          <TextInput
            ...
            placeholder={I18n.t("base.components.assetactiondescription.type-here")}
  ...
  render() {
    const { isSelectAssets, selectedAsset, updatedAssets, selectedAction, isLocationModalOpen, isDateModal,isAssetWarning } = this.state;

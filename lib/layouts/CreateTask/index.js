...
class CreateTask extends React.Component {
  ...
  renderPagePerIndex = (index, displayAssignation) => {
    const { room, locations, assets, isDisableLiteTasks, customActions, assignmentOptions, selectedLocations, assigneeGroup,
      setGroupAssignee, searchAssignee, originalAssigneeGroup, setActiveGroupAssignee } = this.props;
    const { isPriority, isMandatory, isBlocking, isGuestReq, finalAssigns } = this.state;
    const taskProps = {
      room: room,
      locations: locations,
      assets: assets,
  ...
  render() {
    ...
    const { taskError, isSendingTask, isSentTask, searchAssignee, config, assigneeGroup } = this.props;
      ...
      <SafeAreaView style={styles.safeAreaContainer}>
        <View style={styles.rootContainer}>
          <View style={styles.stepIndicator}>
            <StepIndicator
              stepCount={displayAssignation && _.isEmpty(finalAssigns) ? 4 : 3}
              customStyles={stepIndicatorStyles}
              currentPosition={currentPage + 1}
              renderStepIndicator={displayAssignation && _.isEmpty(finalAssigns) ? this.renderStepIndicator : this.getStepIndicatorIconConfigWithoutAssignation}

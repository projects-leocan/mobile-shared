...
import { get, uniq, map, filter, flatten, compact, isEmpty, uniqBy, intersection } from 'lodash';
...
class CreateTask extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedDateTime: { time: { hr: moment(new Date()).format("HH"), min: moment(new Date()).format("mm") }, date: moment(new Date()).format("YYYY-MM-DD") }
    }
  }
  ...
  componentDidMount() {
    ...
    if(this?.props?.room) {
      const finalSelectedArray = this.props?.locations?.map((l) => l.id === this?.props?.room.id ? {...l, isSelected:true} : l)
      this.props.setLocation(finalSelectedArray)
    }
  }
  ...
  validateInput = (position) => {
    return new Promise((resolve, reject) => {
      ...
      const validateLocation = getSelectedLocation.some((s) => s.isSelected)
  ...
  handleSubmit = () => {
    const { getSelectedLocation } = this.props;
    const { isPriority, isGuestReq, isBlocking, selectedDateTime } = this.state;
    ...
    const finalAssignee = compact([...userGroupIds, ...userSubGroupIds, ...userIds])
    const validateLocation = getSelectedLocation.filter((s) => s.isSelected)
    const concatedDate = selectedDateTime?.date + " " + selectedDateTime?.time.hr + ":" + selectedDateTime?.time.min
    const newDate = moment(concatedDate).format("YYYY-MM-DD HH:mm:ss")
    ...
    const data = {
      roomIds: validateLocation.map(a => a.id),
      ...
      startsAt: moment.utc(moment(newDate).utc()).format(),
      userIds: userIds,
  ...
  renderPagePerIndex = (index, displayAssignation) => {
    const { room, locations, assets, isDisableLiteTasks, customActions, assignmentOptions, getSelectedLocation, assigneeGroup,
      setGroupAssignee, searchAssignee, originalAssigneeGroup, setActiveGroupAssignee, isAttendantApp, setLocation, isMaintenanceApp } = this.props;
    const { isPriority, isMandatory, isBlocking, isGuestReq, finalAssigns, isCaptureImage, currentPage } = this.state;
    const taskProps = {
      room: room,
      locations: locations,
      assets: assets,
      ...
      selectedLocations: getSelectedLocation,
      ...
      isMaintenanceApp: isMaintenanceApp,
      isEmptyFinalAssignee: displayAssignation && _.isEmpty(finalAssigns),
      ...
      getSelectedDateTime: (date, value) => this.getSelectedDateTime(date, value)
    };
    ...
    if (isAttendantApp) {
      if (displayAssignation && _.isEmpty(finalAssigns)) {
        switch (index) {
          ...
          case 1:
            return <SelectLocation ref={ref => this.SelectLocation = ref} {...taskProps} setLocation={setLocation} getSelectedLocation={getSelectedLocation} />
          case 2:
  ...
  getSelectedDateTime = (value, date) => {
  ...
  render() {
    ...
    const displayAssignation = get(config, 'showSelectGroupInAttendant', false)
    // if (taskError) {
      ...
      <SafeAreaView style={styles.safeAreaContainer}>
        <View style={styles.rootContainer}>
          <View style={styles.stepIndicator}>
            <StepIndicator
              stepCount={isAttendantApp ? displayAssignation === false && _.isEmpty(finalAssigns) ? 4 : 3 : 4}
              customStyles={stepIndicatorStyles}
              currentPosition={currentPage + 1}
              renderStepIndicator={isAttendantApp ? displayAssignation === false && _.isEmpty(finalAssigns) ? this.renderStepIndicator : this.getStepIndicatorIconConfigWithoutAssignation : this.renderStepIndicator}
...
const mapStateToProps = (state, props) => {
  ...
  return {
    isMaintenanceApp: get(props, 'screenProps.isMaintenanceApp', null),
    ...
    locations: locationsSelectorData(state),
    ...
    getSelectedLocation: getSelectedLocationFilter(state)
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    setLocation: data => dispatch(FiltersActions.setRoomLocation(data)),
    dispatch,
  };
};

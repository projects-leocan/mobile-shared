...
import {
  get, extend, pick, pull, includes, find, uniq, map, keyBy, groupBy,
  at, omit, values, flatten, isEmpty, compact, uniqBy
} from "lodash";
...
class CreateTaskLayout extends Component {
  ...
  _handleSelectAsset = asset => {
    ...
    const existingId = get(this, "state.selectedAsset.id");
    const newId = get(asset, "id");
  ...
  _handleSelectModel = model => {
    ...
    if (
      selectedModel &&
      get(selectedModel, "id", null) === get(model, "id", null)
    ) {

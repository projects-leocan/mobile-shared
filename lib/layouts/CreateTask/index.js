...
class CreateTask extends React.PureComponent {
  ...
  renderPagePerIndex = (index, displayAssignation) => {
    const { room, locations, assets, isDisableLiteTasks, customActions, assignmentOptions, getSelectedLocation, assigneeGroup,
      setGroupAssignee, searchAssignee, originalAssigneeGroup, setActiveGroupAssignee, isAttendantApp, setLocation, isMaintenanceApp, isInspector } = this.props;
    const { isPriority, isMandatory, isBlocking, isGuestReq, finalAssigns, isCaptureImage, currentPage } = this.state;
    let tasks =  this.props.navigation.state.params.tasks;
    const taskProps = {
      room: room,
      locations: locations,
      assets: assets,
    ...
    if (isAttendantApp) {
      if (displayAssignation && _.isEmpty(finalAssigns)) {
        switch (index) {
          ...
          case 1:
            return <SelectLocation ref={ref => this.SelectLocation = ref} {...taskProps} setLocation={setLocation} getSelectedLocation={getSelectedLocation} tasks={tasks} isInspector={isInspector} isMaintenance={isMaintenanceApp}/>
          case 2:
...
const mapStateToProps = (state, props) => {
  ...
  return {
    ...
    isInspector: get(props, 'screenProps.isInspectorApp', null),

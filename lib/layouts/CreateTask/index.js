...
class CreateTask extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedDateTime: { time: moment(new Date()).format("HH:mm"), date: moment(new Date()).format("YYYY-MM-DD") }
    }
  }
  ...
  handleSubmit = () => {
    ...
    const concatedDate = selectedDateTime?.date + " " + selectedDateTime?.time
  ...
  render() {
    const { currentPage, finalAssigns, selectedDateTime } = this.state;

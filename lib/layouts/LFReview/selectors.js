...
export const gropuedFilteredLFSelector = (searchQuery = '', isShowAll = true) => createSelector(
  ...
  (lfItems) => {
    const filtered = lfItems.filter(item => {
      ...
      //   return false;
      // }
      console.log("dfdfdf", `${item.user_first_name}${" "}${item.user_last_name}`);

      if (!isShowAll && ['WaitingRoomMaid'].includes(item.status)) {
        return false;
      }
      ...
      if (searchQuery) {
        ...
        if (
          ...
          !(`${item.user_first_name}${" "}${item.user_last_name}` || '').toLowerCase().includes(cleanQuery) &&

...
import { SectionList, InteractionManager, Platform } from 'react-native';
import Modalbox from 'react-native-modalbox';
import { connect } from 'react-redux';
import _, { get } from 'lodash';

import { groupedHotelLFSelector, gropuedFilteredLFSelector } from './selectors'
...
import Item_M from 'rc-mobile-base/lib/layouts/LFReview/mobileItem';
...
class LFReviewLayout extends React.PureComponent {
  ...
  _handleStatusUpdate = (status) => {
    ...
    this.setState({ selectedStatus: status, isShowItem: status === "hand-delivered" });
  ...
  _handleCloseStatus = () => {
    ...
    const oldUrlChange = activeItem.image_urls.every(r => oldImagesChanges.includes(r))

    InteractionManager.runAfterInteractions(() => {
      if (!_.isEmpty(this.state.newUploadedImage) || oldUrlChange === false) {
      ...
      <Container>
...

        {
          Platform.isPad ?
            ...
            <Content>
              <SectionList
                renderItem={({ item }) => <Item_M item={item} handleActive={this._handleActiveItem} onpressImageIcon={this.onpressImageIcon} isShowImageViewer={isShowImageViewer} setActiveImageId={setActiveImageId} />}
                renderSectionHeader={({ section }) => <ListHeader date={section.title} key={section.title} />}
                sections={this.props.filteredLFItems}
                keyExtractor={(item, index) => item.reference || item.date_ts}
                ListHeaderComponent={() => null}
            ...
            <Modalbox
              ...
              style={{ width: Platform.isPad ? 800 : "90%", height: 600 }}
            >
              <Modal

...
class LFReviewLayout extends React.PureComponent {
  ...
  _handleActiveItem = (item) => this.setState({ activeItem: item, notes: item && item.notes || '', isShowItem: true, photoOnePath: item.image_urls })
  ...
  savePhotos = (field, path) => {
    const { photoOnePath, activeItem } = this.state
    this.setState({ photoOnePath: [...photoOnePath, path] });
  }

  removePhotoOnCloseIcon = (item) => {
    const { photoOnePath } = this.state
  ...
  _handleCloseStatus = () => {
    ...
    //     this.props.updateItemPhoto(imageObject, 'added_image_one', this.state.photoOnePath)
    //   }

...
class PopupLayout extends Component {
  ...
  _renderTask(tasks) {
    ...
    const anonymizeGuestName = this.props.auth?.config?.anonymizeGuestName;
            ...
            {GuestName ?
              <View style={{ width: '100%' }}>
                ...
                <View style={[styles.horizontalContainer]}>
                  <View style={[styles.locationContainer, flexGrow1]}>
                    <View>
                      <Text style={[styles.taskCreatorTitle, { fontWeight: '500', paddingBottom: 5 }]}>{GuestName ? isAttendantApp && anonymizeGuestName ? 'no name' : `${GuestName}` : ''}</Text>
                    </View>
                  </View>
                </View>
              </View>
  ...
  render() {
    ...
    if (!isEmpty(firstTask)) {
      ...
      if(!isEmpty(getUserIds) || !isEmpty(getGroupIds) || !isEmpty(getsubGroupIds))
      {

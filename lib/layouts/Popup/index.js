...
import { last, difference, first } from 'lodash/array';
...
class PopupLayout extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    console.log('--- rc popup ---')
    console.log(newMessage)
    console.log(currentMessage)


    if (newMessage.length !== currentMessage.length) {
      const arrivedMessages = difference(newMessage, currentMessage)
      console.log('---- new message1 ----')
      console.log(arrivedMessages)
      this.setState({ arrivedMessage: arrivedMessages })
    }
  }
  ...
  _renderRoomMessage(messages) {
    ...
    const activeUUID = get(firstMessage, 'id');
      ...
      >
        <View style={styles.headerContainer}>
          <View style={styles.horizontalContainer}>
            <Text style={[styles.headerTitle, { flex: 1 }]}>{`New Message`}</Text>
          </View>
        ...
        <View style={styles.alertBodyContainer}>
          ...
          <View style={styles.horizontalContainer}>
            {mapMessageRoom.map(item => {
              return (
                <View style={[styles.locationContainer, margin.t30, margin.b10, flx1]}>
                  <SimpleLineIcons
                    name="location-pin"
                    size={30}
                  />
          ...
          <View style={styles.bottomContainer}>
            <TouchableOpacity style={[styles.optionBtn, { backgroundColor: '#3CC86B', width: '45%' }]} onPress={() => this._handleTask(activeUUID, 'claimed')}>
  ...
  _renderTask(tasks) {
    ...
    return (
      ...
      >
        <View style={styles.headerContainer}>
          <View style={styles.horizontalContainer}>
            <Text style={[styles.headerTitle, { flex: 0.8 }]}>{I18n.t('base.popup.index.new-task')}</Text>
        ...
        <View style={styles.alertBodyContainer}>
          <Text style={styles.taskTitle}>{asset}</Text>
          ...
          <View style={styles.horizontalContainer}>
            <View style={[styles.locationContainer, margin.t30, margin.b10, flx1]}>
              <SimpleLineIcons
                ...
                size={30}
              />
              <Text style={[styles.bodyTitle, { marginLeft: 12 }]}>{roomName}, {location || "No Location"}</Text>
            </View>

            <View style={[styles.roundedImageContainer, { alignSelf: 'flex-end', justifyContent: 'flex-end' }]}>
              <Image source={bellhop} style={styles.roundedImage} />
            </View>
          </View>
          ...
          <View style={styles.horizontalContainer}>
            <View style={[styles.locationContainer, flexGrow1]}>
              ...
              <View style={{ marginLeft: 10 }}>
                <Text style={styles.creatorTitle}>{I18n.t('base.popup.index.created-by')}</Text>
                <Text style={[styles.creatorTitle, { fontWeight: '500' }]}>{user ? `${user.first_name} ${user.last_name}` : ''}</Text>
              </View>
            </View>

            <View style={{ flexShrink: 1, alignItems: 'flex-end', justifyContent: 'flex-start' }}>
              <Text style={[styles.creatorTitle, { lineHeight: 30 }]}>{moment.unix(date_ts).fromNow()}</Text>
            </View>
          ...
          <View style={{ marginVertical: 18 }}>
            {taskComment
              ?
              <>
                <Text style={[styles.creatorTitle, { fontSize: 18, lineHeight: 35 }]}>Comments</Text>
                <Text style={styles.commentDescription}>{taskComment}</Text>
              </> : null
            }
          </View>
          ...
          <View style={styles.bottomContainer}>
            {!isMandatory ?
              <TouchableOpacity style={[styles.optionBtn, blueDk.bc, { backgroundColor: 'transparent', width: '45%' }]} onPress={() => this._handleTask(activeUUID, 'rejected')}>
                <Text style={[styles.optionBtnText, blueDk.text]}>{I18n.t('base.popup.index.reject').toUpperCase()}</Text>
              </TouchableOpacity>
              : null
            }
            <TouchableOpacity style={[styles.optionBtn, { backgroundColor: '#3CC86B', width: '45%' }]} onPress={() => this._handleTask(activeUUID, 'claimed')}>
  ...
  render() {
    const { tasks, notifications, glitches, assignedAudits, hotelMessages } = this.props;
    ...
    return (
      <Modal
        style={[availableTasks.length ? styles.taskModal : styles.modal]}
        // position={"center"}
        ref={"modal3"}
          ...
          : availableTasks.length ?
            ...
            : glitches.length ?
              ...
              : availableAudits.length ?
                ...
                : hotelMessages.length ?
                  this._renderRoomMessage(hotelMessages) : null
        }
      </Modal>
    )
  }

...
import { computedAttendantPopupTasks, computedAttendentSocketPopupTask } from 'rc-mobile-base/lib/selectors/attendant';
...
class PopupLayout extends Component {
  ...
  _renderTask(tasks) {
    const { indexUsers, isDismissEnable, hotelRooms, isRunnerApp, taskSocket } = this.props;
    const firstTask = get(tasks, 0, {});

    console.log("this.props.taskSocket", taskSocket);
  ...
  render() {
    const { tasks, notifications, glitches, assignedAudits, socketHotelMessages, newTask } = this.props;
    const { viewedTasks, viewedAudits, arrivedMessage } = this.state;
    console.log("this.props.newTask +++++",newTask)

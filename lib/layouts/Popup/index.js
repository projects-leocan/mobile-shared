...
class PopupLayout extends Component {
  ...
  _renderRoomMessage(messages) {
    ...
    console.log("----- mapMessageRoom ----", mapMessageRoom);
  ...
  _renderTaskImages = (item, index, totalLength) => {
    return (
      <View style={[styles.taskImageCell]}>
        <FastImage source={{ uri: item }} style={styles.taskImage} />
      </View>
    )
  }

  _renderTask(tasks) {
    const { indexUsers, isDismissEnable, hotelRooms, isRunnerApp } = this.props;
    ...
    return (
      ...
      >
          ...
          <View style={styles.alertBodyContainer}>
            <View style={styles.taskLabelHeader}>
              <View style={styles.taskLabelLeftContainer} >
                ...
                <View style={[styles.timeLabelContainer]}>
                  <Text style={[styles.fromNowTitle, { lineHeight: 30, marginRight: 10, fontWeight: '700' }]}>{I18n.t('base.popup.index.sent')}</Text>
                  <Text style={[styles.fromNowTitle, { lineHeight: 30 }]}>{moment(created_at).fromNow()}</Text>
                </View>
              </View>
            ...
            <View style={styles.horizontalContainer}>
              <View style={[styles.locationContainer, margin.t20, margin.b20, flx1]}>
                ...
                <View style={{ ...margin.t10, ...margin.b10, flex: 1, flexDirection: 'row' }}>
                  {/* <Text style={[styles.bodyTitle, { marginLeft: 10 }]} numberOfLines={1} adjustsFontSizeToFit>{I18n.t('base.popup.index.location')}</Text> */}
                  <Text style={[styles.bodyTitle, { marginLeft: 12 }]} adjustsFontSizeToFit>{roomName || "No Location"}</Text>
                  <Text style={[styles.bodyTitle, { fontSize: 20, alignSelf: 'center' }]}>{", " + hotelName}</Text>
                </View>
              </View>
            ...
            {GuestName ?
              <View style={[styles.timeLabelContainer]}>
                <Text style={[styles.taskCreatorTitle, { fontWeight: '600' }]}>{I18n.t('base.ubiquitous.guest')}</Text>
                <Text style={[styles.taskCreatorTitle, { fontSize: 16, paddingLeft: 10 }]}>{GuestName ? `${GuestName}` : ''}</Text>
              </View>
              : null}
            ...
            <View style={{ marginBottom: 18 }}>
              {taskComment
                ?
                <>
                  <Text style={styles.commentTitle}>{I18n.t('base.popup.index.comments')}</Text>
              ...
              <View style={[styles.horizontalContainer, { paddingVertical: 18 }]}>
                <View style={[styles.locationContainer, flexGrow1]}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={[styles.taskCreatorTitle, { fontSize: 16 }]}>{userType ? userType : ''}</Text>
                  </View>
                </View>
              </View>
            </View>
        ...
        <View style={styles.bottomContainer}>
          ...
          <TouchableOpacity style={[styles.taskOptionBtn, { backgroundColor: themeTomato.color, width: !isMandatory ? '45%' : '100%' }]} onPress={() => this._handleTask(hotel_id, activeUUID, 'claimed')}>

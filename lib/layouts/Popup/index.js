...
class PopupLayout extends Component {
  ...
  _renderTask(tasks) {
    ...
    const created_at = get(firstTask, 'created_at');
          ...
          <View style={styles.alertBodyContainer}>
            <View style={styles.taskLabelHeader}>
              <View style={styles.taskLabelLeftContainer} >
                ...
                <Text style={[styles.fromNowTitle, { lineHeight: 30 }]}>{moment.unix(created_at).fromNow()}</Text>
              </View>
  ...
  const tasks = props.isAttendantApp ?
    ...
    : props.isMaintenanceApp ?
      ...
      : props.isRunnerApp ?
        ...
        :
        computedHotelsPopupTasks(state);

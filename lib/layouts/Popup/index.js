...
class PopupLayout extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    return !checkEqual(taskIds, nextTaskIds)
      ...
      || !checkEqual(this.state, nextState, 'arrivedMessage');
  }
  ...
  onDismissMessage = (activeID) => {
    const { arrivedMessage } = this.state;
    const updateMessages = arrivedMessage.filter(item => item.id !== activeID);

    this.setState({ arrivedMessage: updateMessages })
  }
  ...
  render() {
    ...
    return (
      <Modal
        style={[(availableTasks.length || arrivedMessage.length) ? styles.taskModal : styles.modal]}
        // position={"center"}
        ref={"modal3"}
          ...
          : availableTasks.length ?
            ...
            : glitches.length ?
              ...
              : availableAudits.length ?
                ...
                : arrivedMessage.length ?
                  this._renderRoomMessage(arrivedMessage) : null
        }
      </Modal>
    )
  }

...
class InventoryLayout extends Component {
  ...
  getRoomInventory = async () => {
    ...
    const mapInventoryMinibarData = map(inventorySets, function (obj) {
      const minibarAssets = map(get(obj, 'minibarAssets', []), function (minibarAssets) {
        return { ...minibarAssets, isUpdate: false, update: 0, isGroupedAsset: false, isMinibarAsset: true}
      })
       ...
       groupedAssets = map(get(obj, 'groupedAssets', []), function (groupedAsset) {
          const regularAssets = !_.isEmpty(groupedAsset.regularAssets) && groupedAsset.regularAssets.map((regularAsset) => {
            ...
            id = groupedAsset.categoryId
              return { ...regularAsset, isUpdate: false, update: 0, isGroupedAsset: true, isMinibarAsset: false};
          });
          return {...obj,assets:regularAssets, isSetSelected: false, categoryName: categoryName, id: id};
      });
  ...
  componentWillUnmount() {
    ...
    const { room, inventoryAssetConsumationUpdate, groupedInventoryAssetConsumationUpdate } = this.props;
    ...
    const mapScheduledAssets = map(filterSelectedSet, function (sets) {
      ...
      category = get(sets, 'categoryName', null);
      ...
      const mapScheduledSetAssets = map(filterScheduledSetAssets, function (asset) {
        const isGroupedAsset = get(asset, 'isGroupedAsset', false);
        const isMinibarAsset = get(asset, 'isMinibarAsset', false);
        if (isMinibarAsset === true) {
          return {
    ...
    const scheduledMinibarAssetsPayload = {
      hotelId: hotelId,
      roomId: roomId,
      scheduledAssets: flatten(mapScheduledAssets)
    }

    const scheduledGroupedAssetsPayload = {
      roomId: roomId,
      inventoryReviewAssets: flatten(mapScheduledAssets)
    }

    if (!isEmpty(flatten(mapScheduledAssets))) {
      if (category === 'Minibar') {
        inventoryAssetConsumationUpdate(scheduledMinibarAssetsPayload)
      }else{
        groupedInventoryAssetConsumationUpdate(scheduledGroupedAssetsPayload)
      }
    }
  }
  ...
  _handleBack = () => {
    ...
    const updateInventoryData = inventoryUpdates.updateInventoryData || [];
    const updatedInventory = updateInventoryData.flatMap(data => data.assets || []).filter(item => item.isUpdate === true);
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    groupedInventoryAssetConsumationUpdate: (groupedAssetConsumationPayload) => dispatch(UpdatesActions.groupedInventoryAssetConsumationUpdate(groupedAssetConsumationPayload)),
    dispatch
  }
};

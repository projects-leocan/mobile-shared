...
class InventoryLayout extends Component {
  ...
  getRoomInventory = async () => {
    ...
    const mapInventoryGroupAssetsData = map(inventorySets, function (obj) {
      let categoryName 
      let id
      const groupedAssets = map(get(obj, 'groupedAssets', []), function (groupedAsset) {
      ...
      const flattenedAssets = _.flatten(groupedAssets); 
      return { ...obj, isSetSelected: false, assets: flattenedAssets, categoryName: categoryName, id: id};
  });
    
    if (!isEmpty(inventorySets)) {
      // this.setState({ inventorySets: mapMinibarInventoryData})
      this.setState({ inventorySets: [...mapInventoryMinibarData , ...mapInventoryGroupAssetsData]})
    }
  }

...
class InventoryLayout extends Component {
  ...
  getRoomInventory = async () => {
    ...
    const inventorySets = await externalAPIService.getInventorySetAndAssets(payload);
    console.log('--- inventorySets ---', inventorySets)
    console.log(payload)

    const mapInventoryData = map(inventorySets, function (obj) {
    ...
    if (!isEmpty(inventorySets)) {
      this.setState({ inventorySets: mapInventoryData })
    }
  }
  ...
  _handleBack = () => {
    ...
    const confirmationItems = inventorySets.map((asset, index) => {
      const assetId = get(asset, 'assets[0].id');
      return get(roomUpdates, assetId) &&{
        image: get(asset, `assets[${index}].imageUrl`),
        name: get(asset, `assets[${index}].name`),
        change: get(asset, `assets[${index}].update`),
        isWithdrawal: true
      }
    }).filter(Boolean);
  ...
  render() {
    ...
    // console.log('--- activeRoomArea ---', activeRoomArea)

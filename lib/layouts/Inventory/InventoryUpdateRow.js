...
class InventoryUpdateRow extends Component {
  ...
  render() {
    const { data, index, activeRoomArea, roomWithdrawals, adjustInventory, rejectInventory, resetInventory, onScroll } = this.props;
    ...
    const count = (isFinite(Number(data?.fullQuantity)) ? Number(data?.fullQuantity) : 0)  - (data.update || 0);
      ...
      <Swipeable
        ...
        <View style={[styles.container, { backgroundColor: index % 2 ? '#F7F7F7' : '#EDEDED' }]}>
          <View style={styles.imageContainer}>
            <Image style={styles.assetImage} source={{ uri: data?.imageUrl }} resizeMethod='resize' />
          </View>
          <View style={[styles.labelContainer , {width: width - 120}]}>
            <View style ={styles.labelContainerText}>
          ...
          <View style={[styles.btnContainer, { backgroundColor: data?.index % 2 ? '#E2E4E5' : '#D9DBDC' }, isSent && green.bg, isUpdate && orange.bg]}>
            <TouchableOpacity style={styles.adjustBtn} onPress={() => adjustInventory(activeRoomArea, data)}>
              { isUpdate ?

...
class LostFound extends Component {
      ...
      return (
        <ExtraBackgroundContainer>
          ...
          {photo.path ?
            ...
            :
            <ExtraExitContainer onPress={this._setPhoto}>
              <Entypo name="cross" size={24} color="black" />
            </ExtraExitContainer>
          }
        </ExtraBackgroundContainer>
      ...
      <View style={styles.outerContainer}>
        ...
        >
          <View style={{ flex: 1, height: '100%', width: '100%' }}>
            <View style={[styles.listContainer, { alignItems: photos.length > 1 ? 'center' : 'flex-start', justifyContent: photos.length > 1 ? 'center' : 'flex-start' }]}>
              {photos.length > 1
                ?
                <>
                  {photos.map((item, index) => {
                    return (
                      <>
                        {item.photoId === 'initial'
                          ?
                          <TouchableOpacity style={styles.imageItemContainer} onPress={() => this.toggleCamera()}>
                          ...
                          <View style={styles.imageItemContainer}>
                            <View style={styles.cancelItemContainer}>
                              <TouchableOpacity style={styles.centerContainer}>
                                <Ionicons name="close" size={30} color={Colors.red} style={{ alignSelf: 'center' }} />
                              </TouchableOpacity>
                            </View>
                            <Image
  ...
  _handleAddLostItem = () => {
    ...
    this.props.addLostItem(photoId, insertLFPayload);
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    addLostItem: (photoId, insertLFPayload) => dispatch(UpdatesActions.lostItemAdd(photoId, insertLFPayload)),
  }
};

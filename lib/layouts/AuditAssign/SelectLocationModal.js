...
const onSelectRooms = (value ,onPress ,selectedRooms) => {
  onPress(value)
  selectedRooms(value)
}
...
export const Opener = ({ location, onPress }) => (
  <TouchableOpacity
    style={[white.bg, flxRow, aic, jcsb, padding.x15, { height: 50 }]}
    onPress={onPress}
  >
    { location ?
      <Text style={{ color: '#808080'}}>{ location.name }</Text>
      :
      <Text style={{ color: '#808080'}}>{ I18n.t('base.ubiquitous.select-location') }</Text>
    }
...
export const SelectLocationModal = ({ selectedRooms ,label, options, ...props }) => (
  <ModalField
    {...props}
    ...
    renderModal={(onPress) => (
      <View style={[flexGrow1, grey.bg]}>
        ...
        <SelectLocation
          options={options}
          // onPress={onPress}
          onPress={(value) => onSelectRooms(value , onPress ,selectedRooms)}
        />
...
const Form = ({ handleSubmit, locations ,selectedRooms }) => (
  <View>

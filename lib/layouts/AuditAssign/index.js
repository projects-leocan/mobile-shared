...
import { locationsSelector, assignmentSelector } from '../CreateTask_/selectors';
...
class AuditAssignScene extends Component {
  ...
  navigateAudit = () => {
    ...
    const { setLocation ,selectedRoom } = this.state;
    ...
    const auditData = this.props.navigation.state.params.audit
    ...
    // createAuditInsertRoute(finalData)
    return navigation.navigate('AuditEdit', { audit: finalData })
  }
      ...
      <Container testID="assignAudit" narrow={narrow}>
        ...
        <Content>
          ...
          {setLocation ?
            ...
            :
            <SelectLocationModal
              selectedRooms={(data) => this.selectedRooms(data)}
              locations={locations}
            />
          }

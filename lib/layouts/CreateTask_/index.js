...
import { get, extend, pick, pull, includes, find, uniq, map, keyBy, groupBy,
  at, omit, values, flatten } from "lodash";
...
class CreateTaskLayout extends Component {
  ...
  _handleSubmit = () => {
    ...
    const { taskImages, selectedAssignments, recordPath, duplicateTaskImages, selectedAsset, selectedAction, assetsHolder, isCaptureModeImage, photoId } = this.state;
    ...
    const selectedAssignmentsHolder = groupBy(at(keyBy(assignmentOptions, 'value'), selectedAssignments), "type")
    ...
    const userIds = map(uniq(flatten(values(omitGroup))), 'value') 
    const userGroupIds = map(get(assignmentsGroups, 'USER_GROUP'), 'value');
    const userSubGroupIds = map(get(assignmentsGroups, 'USER_SUB_GROUP'), 'value');

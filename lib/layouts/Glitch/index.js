...
class GlitchLayout extends Component {
  ...
  _handleSubmit = (values) => {
    ...
    !_.isEmpty(this.props.options) && this.props.options.map((Categorie) =>{
      if (!_.isEmpty(Categorie.options)) {
        !_.isEmpty(Categorie.options) && Categorie.options.map((subcategory)=>{
          if (values.category === subcategory.withMainCategory) { 
            experienceCategoryId =  subcategory.id
          }
        })
      }else{
         values.category === Categorie.name ?  experienceCategoryId = Categorie.id :  experienceCategoryId = ""
      }
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchGlitch: () => dispatch(GlitchesActions.glitchesOptionsFetch()),
    dispatch
  }
};

...
class GlitchForm extends Component {
  ...
  _required = (value, values) => {
    this.setState({ roomValue: values })
  }

  _handleSubmit = () => {
    console.log("inside _handleSubmit");
    const { handleSubmit, changeClose } = this.props;
    const { roomValue } = this.state
    if (!('room_name' in this.state.roomValue)) {
      return this.setState({ roomValue: { ...roomValue, room_name: "" } })
    }

    if (roomValue.room_name) {
      // changeClose(isClose);
      handleSubmit();
    }
  }
      ...
      <View style={styles.container}>
        <ScrollView contentContainerStyle={{paddingBottom:50}}>
          ...
          <Section title={I18n.t('runner.glitch-form.index.guest-information')} style={{ marginTop: 20 }}>
            <View style={styles.row}>
              ...
              <Field
                ...
                validate={this._required}
              />
            </View>
            {
              this.state.roomValue.room_name || !('room_name' in this.state.roomValue) ? <></> : <Text style={{ color: "red", marginLeft: 90 }}>{I18n.t('runner.glitch-form.experience-form.select-room-number')}</Text>
            }
            <View style={styles.row}>
          ...
          <View style={styles.submitButtonContainer}>
            {/* <TouchableOpacity style={[styles.submitButton, isEditing && props.dirty ? Colors.blueLt.bg : null]} onPress={props.pristine ? onClose : this._handleSubmit }>
              { isEditing ?
                props.pristine ?

...
class GlitchesLayout extends Component {
  ...
  _handleGlitchNavigation = (uuid, roomId) => {
    let allGlitches = this.props.availableGlitches
    let getGlitch = !_.isEmpty(allGlitches) && allGlitches.filter((data) => {
      if (data.id === uuid) {
        return data
      }
    })
    let data = getGlitch.length > 0 ? getGlitch[0] : []
    ...
    this.props.navigation.navigate('Glitch', {data});
  }

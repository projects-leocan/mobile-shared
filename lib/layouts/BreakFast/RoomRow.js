...
class RoomRowItem extends Component {
  ...
  render() {
    ...
    const allBreakfast = this.props.allBreakfast
    ...
    let breakfast  = !_.isEmpty(activeRoomCalender) && activeRoomCalender[0]?.otherProperties?.filter((data) => {
      if (data?.key === "product_name" && ((data?.value.includes("PDJ")) || (data?.value.includes("ALLCOUNT")))) {
        return data?.value
      }
    })

    let reservation_id = get(activeRoomCalender[0], 'id', "")
    let cameToBreakFast = !_.isEmpty(allBreakfast) && allBreakfast.filter((data) =>{
    ...
    return (
      <TouchableOpacity activeOpacity={0} style={[styles.container,{backgroundColor: !_.isEmpty(cameToBreakFast) ? "" : "white"}]} onPress={() => this.props.roomNavigation(id)}>

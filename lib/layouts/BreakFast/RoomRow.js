...
class RoomRowItem extends Component {
  ...
  render() {
    ...
    let test = "PDJB INC|TPT|TXR|TXS".replace('|' , ' ')
    ...
    let breakfast  = !_.isEmpty(roomCalendar) && roomCalendar[0]?.otherProperties?.filter((data) => {
      if (data?.key === "product_name") {
        return data?.value.includes("PDJ")
      }
    })
    ...
    return (
      <TouchableOpacity activeOpacity={.1} style={[styles.container]} onPress={() => this.props.roomNavigation(id)}>
        ...
        <View style={[styles.roomIconsContainer]}>
          ...
          {breakfast ? 
          <View style={[styles.breakfastIconView, { backgroundColor: breakfast ? '#c1e4c3' : 'white' }]}>

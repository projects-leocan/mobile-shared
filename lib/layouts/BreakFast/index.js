...
class BreakFast extends Component {
  ...
  _renderFloorRooms() {
    ...
    let allBreakfast = this.props.updatedBreakfastArray
      ...
      return data?.roomCalendar?.map((item) => {
        if (!(item?.status === "arrival" || item?.status === "arrived") && (!(item.occupants === 0))) {
          !_.isEmpty(item?.otherProperties) && item.otherProperties.map((product) => {
            ...
            if (product?.key === "product_name" && ((brekfastValue?.includes("PDJ")) || (brekfastValue?.includes("ALLCOUNT")) || (brekfastValue?.includes("PETIT")))) {

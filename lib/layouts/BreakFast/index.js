      ...
      return data?.roomCalendar?.map((item) => {
        if (item?.status != "arrival" || item?.status != "arrived") {
          !_.isEmpty(item?.otherProperties) && item.otherProperties.map((product) => {
            if (product?.key === "product_name" && product?.value.includes("PDJ")) {
    ...
    return (
      <View style={styles.rowsView}>
        <View style={styles.countView}>
          <Text style={styles.breakfastCountText}>{uniq(breakfastArr).length + ' rooms / ' + totalRooms?.length + ' rooms'}</Text>

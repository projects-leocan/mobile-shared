  ...
  _renderFloorRooms() {
    ...
    let roomCalendar = !_.isEmpty(selectedFloorRooms) && selectedFloorRooms.map((data) => {
      return data?.roomCalendar?.map((item) => {
        if (item?.status != "arrival" || item?.status != "arrived") {
    ...
    return (
      <View style={styles.rowsView}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <RoomRow room={item} isEnableAdvancedMessages={this.props.isEnableAdvancedMessages} roomNavigation={this._handleRoomNavigation} allBreakfast={this.props.allBreakfast} />}

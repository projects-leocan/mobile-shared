...
import { allAudits } from '../Audits/selectors'
...
export const getById = (id) => createSelector(
  [Audits.getById(id), allAudits, userSelector, hotelIdSelector],
  (audit, auditSources, user, hotelId) => {
    const auditObj = auditData[0]

    const mapAuditSource = auditSources.find(as => as.id.toString() === id.toString())
    if (!mapAuditSource) {
      return null
    }
    const responder_id = get(mapAuditSource, 'CreatedBy', null);
    const responder_name = get(mapAuditSource, 'auditCreater.username', "")
    const hotel_id = hotelId || null

    // let inspections = mapAuditSource.sections.map((inspection, index) => {
    //   const existing = (audit.inspections || []).find(i => i.inspection_source_id === inspection.id)
    ...
    return {
      ...
      hotel_id,
      // inspections,
      inspections: get(mapAuditSource, 'sections', []),
      isExisting: true,
    }
  }
)
...
import { isEmpty, get, map, remove, sortBy } from 'lodash';
...
class GalleryLayout extends Component {
  ...
  getRoomCatalog = async () => {
    let catalogList = get(this, 'props.navigation.state.params.catalogHolder', [])
    let imageIndex = get(this, 'props.navigation.state.params.imageIndex', [])
    if (isEmpty(catalogList)) {
      catalogList = get(this, 'props.catalogHolder', []);
    }
    if (!isEmpty(catalogList)) {
  ...
  renderViewPagerPage = (data, index) => {
    ...
    const currentPageDescription = get(sortedCatalogHolder[currentIndex], 'annotations', []);
    return (
      <>
        ...
        <PageContainer key={index.toString()} activeOpacity={1}>
        {
          data?.name ?
            <Text
              style={get(screenProps, 'isAttendantApp', false) || get(screenProps, 'isRunnerApp', false) || get(screenProps, 'isMaintenanceApp', false) ? {
                padding: 5,
                fontSize: 20
              }
                :
          ...
          <TouchableOpacity onPress={() => this.toggleTaskImageModal(index)}>
        ...
        <RootContainer>
          <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
            <HeaderImageSliderContainer>
              ...
              >
                {sortedCatalogHolder.map((page, index) => this.renderViewPagerPage(page, index))}
              </PagerView>
            ...
            <ImageViewer
              ...
              initialIndex={this.state.imageViewIndex}
              images={map(sortedCatalogHolder, function (obj) { return { url: get(obj, 'annotatedImageUrl', null) } })}
              toggleTaskImageModal={() => this.toggleTaskImageModal(this.state.imageViewIndex)}
            />

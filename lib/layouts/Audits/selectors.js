...
import { hotelIdSelector, userIdSelector } from '../../selectors/auth';

import { Audits, AuditSources } from 'rc-mobile-base/lib/models';
import auditData from './audit.json'
import { usersSelector, auditsSelector } from 'rc-mobile-base/lib/selectors/rooms';
import { filter, find, get, map } from 'lodash';

export const allAudits = createSelector(
  [auditsSelector, usersSelector, userIdSelector, hotelIdSelector],
  (audits, users, userId, hotelId) => {
    const selectedAudit = []
    const allHotelAudits = []
    filter(audits, function(audit){
      if (audit.hotelId === hotelId) {
        selectedAudit.push(audit)
      }else if(audit.hotelId === null){
        allHotelAudits.push(audit)
      }
    })
    const allAuditsData = [...selectedAudit, ...allHotelAudits]
    const mapAudits = map(allAuditsData, function(audit) {
      return {
        ...audit,

...
import { activeTabSelector, sentTasks, assignedTasks, backlogTasks, closedTasks, finishTask } from './selectors';
...
class TasksLayout extends Component {
  ...
  render() {
    const { assigned, backlog, sent, activeTab, tabToggle, user, closedTask, finishTasks } = this.props;
    const isSendDisabled = get(this, 'props.screenProps.isTaskSendDisabled') || false;
    const userType = get(user, 'roleName', '');
    let allTasks = [...assigned, ...finishTasks]
    return (
      <Screen>
        <Header>
          <Tabs
        ...
        <Tab
          ...
          active={activeTab}
        >
          <ListTasks {...this.props} tasks={sent} activeTab={2} />
        </Tab>
        ...
        <Tab
          ...
          active={activeTab}
        >
          <ListTasks {...this.props} tasks={closedTask} activeTab={3} />
        </Tab>

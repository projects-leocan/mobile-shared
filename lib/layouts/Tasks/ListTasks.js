...
const ListTasks = ({ tasks, isSectionList, onSwipeoutPress, navigation }) => {
  ...
  return(
    ...
    renderTask={(task) => <TaskRowExpandable key={task.uuid} task={task} onSwipeoutPress={onSwipeoutPress} onTaskDetail={(task) => navigation.navigate('ExpandedTaskDetail', { task: task })}/>}
  />
  )
}

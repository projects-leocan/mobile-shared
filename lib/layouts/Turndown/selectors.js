...
export const getUserNightRoomsByFloor = (hotelFloorsWithRooms, planningsNight, userId) => {
  ...
  if (planningsNight && planningsNight.length) {
    ...
    return hotelRooms
      ...
      .map(room => ({
        ...room,
        roomPlanning: get(userRoomsIndex, room.id, {})
      }))
  }
...
export const getUserNightRoomsByFloorInspector = (hotelFloorsWithRooms, planningsNight, userId) => {
  if (!hotelFloorsWithRooms || !hotelFloorsWithRooms.length) {
    return [];
  }
  ...
  if (planningsNight && planningsNight.length) {
    ...
    const userRoomsIndex = keyBy(planningsNight, 'room_id');
...
export const computedUserNightRoomsByFloorInspector = createSelector(
  [computedAllFloors, planningsNightSelector, userIdSelector],
  getUserNightRoomsByFloorInspector
)
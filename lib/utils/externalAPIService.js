...
const ROOM_MINIBAR_INVENTORY_API = `/InventoryReview/GetRoomInventoryAssets`
...
export async function getInventorySetAndAssets(payload) {
    ...
    const options = {
        ...
        body: JSON.stringify({
            hotelId: hotelId,
            roomId: "50daeb5d-a922-4e45-8359-4f706e7f70f4"
        })
    }
...
export async function getMinibarInventory(payload) {
    const { token, hotel_group_key, baseUrl, roomIds, hotelId } = payload;
    const headers = {
        Authorization: `Bearer ${token}`,
        hotel_group_key: hotel_group_key,
        'Content-Type': 'application/json'
    };
    const url = `${baseUrl}${ROOM_MINIBAR_INVENTORY_API}`
    const options = {
        method: 'POST',
        body: JSON.stringify({
            hotelId: hotelId,

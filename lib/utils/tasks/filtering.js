...
export function isAssignedUser(task, user, plannings = []) {
  const { id: userId, isAttendant, isRoomRunner, isInspector, isMaintenance } = user;
  if (!_checkTask(task, userId)) {
    return false;
  }
...
export function isSentAssignedUser(task, user, plannings = []) {
  // const { id: userId, isAttendant, isRoomRunner, isInspector, isMaintenance } = user;
  // if (!_checkTask(task, userId)) {
  //   return false;
  // }
...
export function isAssignedToday(task, user, plannings) {
  ...
  return get(task, 'start_date') === today;
}

export function isSentAssignedToday(task, user, plannings) {
  if (!isSentAssignedUser(task, user, plannings)) {
    return false;
  }
...
export function isGuestRequest(task, userId) {
  return (get(task, 'is_guest_request') && isAssigned(task, userId));
};
...
export const checkPriority = (task, userId) => {
  if (_checkTask(task, userId) && (isPriority(task, userId) || isGuestRequest(task, userId))) {
    return true
  }
  return false
}
...
export const checkNormal = (task, userId) => {
  if (_checkTask(task, userId) && !isPriority(task, userId) && !isGuestRequest(task, userId) && isAssigned(task, userId)) {
    return true
  }
  return false
}
...
export const checkRecurringAll = (task) => {
  if (_checkTaskAll(task) && (get(task, 'typeKey', "") === "RECURRING") && !get(task, 'is_verified')) {
    return true;
  }
  return false;
}

export const checkVerifiedAll = (task) => {
  if (_checkTaskAll(task) && (get(task, 'is_verified', false) === true)) {
    return true;
  }
  return false;
}
...
export const checkPriorityAll = (task) => {
  if (_checkTaskAll(task) && (get(task, 'is_priority') || get(task, 'is_guest_request')) && !get(task, 'is_verified')) {
    return true;
  }
  return false;
}
...
export const checkNormalAll = (task) => {
  if (_checkTaskAll(task) && !isPriority(task) && get(task, 'responsible_id') && !(get(task, 'typeKey', "") === "RECURRING") && !get(task, 'is_verified')) {
    return true;
  }
  return false;
}
...
export const checkUnassignedAll = (task) => {
  if (_checkTaskAll(task) && !isPriority(task) && isUnassigned(task) && !(get(task, 'typeKey', "") === "RECURRING") && !get(task, 'is_verified')) {
    return true
  }
  return false
} 
...
export const cencelled = filterBy(isCancelled)
...
export const assignedSentToday = filterBy(isSentAssignedToday)

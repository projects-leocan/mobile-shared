...
export const checkRecurringAll = (task) => {
  if (_checkTaskAll(task) && (get(task, 'typeKey', "") === "RECURRING")) {
    return true;
  }
  return false;
}
...
export const checkPriorityAll = (task) => {
  if (_checkTaskAll(task) && (get(task, 'is_priority') || get(task, 'is_guest_request'))) {
    return true;
  }
  return false;
}
...
export const checkNormalAll = (task) => {
  if (_checkTaskAll(task) && !isPriority(task) && get(task, 'responsible_id') && !(get(task, 'typeKey', "") === "RECURRING")) {
    return true;
  }
  return false;
}
...
export const checkUnassignedAll = (task) => {
  if (_checkTaskAll(task) && !isPriority(task) && isUnassigned(task) && !(get(task, 'typeKey', "") === "RECURRING")) {
    return true
  }
  return false
} 

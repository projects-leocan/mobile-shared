...
class Data {
  ...
  load() {
    // this.dispatch(RoomsActions.socketFireLoader(true))
    const { options } = this;
    InteractionManager.runAfterInteractions(() => {
      ...
      //this.dispatch(RoomsActions.addSocketList("", "app refresh", "all API"))

    });
    setTimeout(() => {
      this.dispatch(RoomsActions.socketFireLoader(false))
      return;
    },15000);
  }
  ...
  refreshData() {
    ...
    InteractionManager.runAfterInteractions(() => {
      ...
      if (!options.disableInspectionClean) {
        this.dispatch(RoomsActions.roomInspectionCleanFetch());
      }
      if (!options.disableRoomNotes) {
      ...
      if (!options.disableAvailHotelsRooms) {
        this.dispatch(HotelsTaskActions.availHotelsRoomFetch());
      }
      if (!options.disableTasks) {
        // this.dispatch(BackendActions.tasksBackend());
        this.dispatch(RoomsActions.tasksFetch())

...
import {moment as moment1} from 'moment-timezone'
...
export function unixPrettyDate(ts, createTaskTime) {
    let today = moment();
    let yesterday = moment().subtract(1, 'day');
    let mValue = moment.unix(ts);
    let taskTime = createTaskTime.split(' ')

    if (today.isSame(mValue, 'day')) {
      return _.isEmpty(taskTime) ? '' : taskTime[1]

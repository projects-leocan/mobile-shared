import React from 'react';
import { View, Image, StyleSheet } from 'react-native';
import { get } from 'lodash/object';
import { take, first, uniq, last, flatten } from 'lodash/array';
import { isEmpty } from 'lodash/lang';
import { includes, filter } from 'lodash/collection';
import moment from 'moment';
import { excludeFutureArrivals } from 'rc-mobile-base/lib/utils/guest-status-by-date';
import silverImage from '../images/membershipImages/silver.png';
import turquoiseImage from '../images/membershipImages/turquoise.png';
import platiniumImage from '../images/membershipImages/platinium.png';
import goldImage from '../images/membershipImages/gold.png';

let displayMap = {
	arrival: `TODAY ARR`,
	arrived: 'ARR',
	stay: 'STAY',
	departed: 'DEP',
	departure: 'DEP',
	future_arrival: 'ARR FUTURE'
};

function sortCalendar(a, b) {
	let s = get(a, 'check_out_date');
	let e = get(b, 'check_out_date');

	if (!s) {
		return -1;
	}
	if (!e) {
		return 1;
	}

	return moment(e).unix() - moment(s).unix();
}

function momentizeDate(date) {
	if (!date) {
		return moment();
	}

	// return moment.utc(date).add(180, 'minute');
	return moment(take(date, 10).join(''));
}

function isMatchGuest(e1, e2) {
	if (!e2) {
		return false;
	}

	let first = get(e1, 'name');
	let second = get(e2, 'name');

	if (!first || !second) {
		return false;
	}

	if (get(e1, 'checkInDate') !== get(e2, 'checkInDate') || get(e1, 'checkOutDate') !== get(e2, 'checkOutDate')) {
		return false;
	}

	let isPartMatch = false;
	get(e1, 'name').split(' ').forEach(function (e1Part) {
		get(e2, 'name').split(' ').forEach(function (e2Part) {
			if (e1Part.toLowerCase() === e2Part.toLowerCase()) {
				isPartMatch = true;
			}
		});
	});

	return isPartMatch;
}

export function calculateGuest(entries, messagesForReservation) {
	let validateEntries = excludeFutureArrivals(entries);

	if (!validateEntries || isEmpty(validateEntries)) {
		return null;
	}

	let today = moment(moment().format('YYYY-MM-DD'));

	validateEntries = validateEntries.reduce(function (pv, item) {
		function calculateScore(r) {
			let score = 0;

			score += moment(get(r, 'check_out_date')).unix();
			if (r.arrival_ts) {
				score += parseInt(get(r, 'arrival_ts'));
			}
			if (r.departure_ts) {
				score += parseInt(get(r, 'departure_ts'));
			}

			return score;
		}

		let guest = get(item, 'guest_name', '').trim().toLowerCase();
		// let seen_guest = pv.map(function (res) {
		// 	return get(res, 'guest_name', '').trim().toLowerCase();
		// });

		// if (seen_guest.indexOf(guest) !== -1) {
		// 	let comparisonIndex = seen_guest.indexOf(guest);
		// 	let pre = calculateScore(pv[comparisonIndex]);
		// 	let post = calculateScore(item);

		// 	if (post > pre) {
		// 		pv[comparisonIndex] = item;
		// 	}
		// } else {
		// 	pv.push(item);
		// }
		pv.push(item);

		return pv;
	}, []);

	let mapped = validateEntries.sort(sortCalendar).map(function (entry) {
		const messageForCurrent = filter(messagesForReservation, function (item) {
			return get(item, 'messageReservationIds', []).includes(get(entry, 'id'));
		});

		let guestData = {
			guest_id: get(entry, 'id'),
			name: get(entry, 'guest_name', '').replace(/ +/g, ' '),
			group: get(entry, 'group_name'),
			occupants: get(entry, 'occupants'),
			adults: get(entry, 'adults'),
			children: get(entry, 'children'),
			infants: get(entry, 'infants'),
			pmsId: get(entry, 'pms_id'),
			pmsNote: get(entry, 'pms_note'),
			rcNote: get(entry, 'rc_note'),
			eta: get(entry, 'expected_arrival_ts'),
			etd: get(entry, 'expected_departure_ts'),
			etaDate: get(entry, 'expected_arrival_date'),
			etaTime: get(entry, 'expected_arrival_time'),
			etdDate: get(entry, 'expected_departure_date'),
			etdTime: get(entry, 'expected_departure_time'),
			vip: get(entry, 'vip', null),
			email: get(entry, 'email', null),
			guest: get(entry, 'guest', {}),
			status: get(entry, 'status', null),
			display: null,
			checkInDate: null,
			checkOutDate: null,
			isActive: false,
			isFutureArr: false,
			reservationMessage: isEmpty(messageForCurrent) ? null : flatten(messageForCurrent.map(item => get(item, 'message', ''))),
			reservationMessageObject: isEmpty(messageForCurrent) ? null : flatten(messageForCurrent)
		};

		let checkInDate = momentizeDate(get(entry, 'check_in_date'));
		let checkOutDate = momentizeDate(get(entry, 'check_out_date'));

		// let isArrToday = checkInDate.format('YYYY-MM-DD') === today.format('YYYY-MM-DD');
		// let isDepToday = checkOutDate.format('YYYY-MM-DD') === today.format('YYYY-MM-DD');
		let isArrFuture = checkInDate.format('YYYY-MM-DD') > today.format('YYYY-MM-DD');

		// let isArrivalExist = get(entry, 'arrival_ts');
		// let isDepartureExist = get(entry, 'departure_ts');

		guestData.checkInDate = checkInDate.format('YYYY-MM-DD');
		guestData.checkOutDate = checkOutDate.format('YYYY-MM-DD');

		// guestData.status = 'stay';

		if (isArrFuture) {
			// guestData.status = 'future_arrival';
			guestData.isFutureArr = true;
		}

		// if (isArrToday) {
		// 	if (isArrivalExist) {
		// 		guestData.status = 'arrived';
		// 	} else {
		// 		guestData.status = 'arrival';
		// 	}
		// }

		// if (isDepToday) {
		// 	if (isDepartureExist) {
		// 		guestData.status = 'departed';
		// 	} else {
		// 		guestData.status = 'departure';
		// 	}
		// }

		guestData.display = displayMap[guestData.status];
		return guestData;
	});

	return mapped.reduce(function (pv, item) {
		let lastPMS = pv.length && (get(pv[pv.length - 1], 'pmsId', '') || '').trim();
		let lastName = pv.length && (get(pv[pv.length - 1], 'name', '') || '').trim();

		let currentPMS = item && (get(item, 'pmsId', '') || '').trim();
		let currentName = item && (get(item, 'name', '') || '').trim();

		let last = lastName + ':' + lastPMS;
		let current = currentName + ':' + currentPMS;

		let isLastMatch = pv.length && isMatchGuest(item, pv[pv.length - 1]);
		// if (item.name.toLowerCase() === "MICHEL HADDAD".toLowerCase() && pv.length > 0) {
		//   debugger;
		// }


		// if (current && last !== current && !isLastMatch) {
		pv.push(item);
		// }


		return pv;
	}, []);
}

export function calculateGuestCode(roomCalendar, roomCalculation) {
	if (!roomCalendar || isEmpty(roomCalendar)) {
		return null;
	}

	if (!roomCalculation) {
		return null;
	}

	let f = first(roomCalculation);

	if (roomCalendar.length === 1) {
		let status = get(f, 'status');

		if (status === 'stay') {
			return { guestStatus: 'stay', guestExactStatus: status };
		}
		if (status === 'departure' || status === 'departed') {
			return { guestStatus: 'dep', guestExactStatus: status };
		}
		if (status === 'arrival' || 'arrived') {
			return { guestStatus: 'arr', guestExactStatus: status };
		}
		if (status === 'future_arrival') {
			return { guestStatus: 'future arr', guestExactStatus: status };
		}

		return { guestStatus: '', guestExactStatus: '' };
	}

	let statuses = roomCalculation.map(function (entry) {
		let status = get(entry, 'status');

		if (status === 'stay') {
			return { guestStatus: 'stay', guestExactStatus: status };
		}
		if (status === 'departure' || status === 'departed') {
			return { guestStatus: 'dep', guestExactStatus: status };
		}
		if (status === 'arrival' || 'arrived') {
			return { guestStatus: 'arr', guestExactStatus: status };
		}
		if (status === 'future_arrival') {
			return { guestStatus: 'future arr', guestExactStatus: status };
		}

		return { guestStatus: '', guestExactStatus: '' };
	});

	statuses = uniq(statuses);
	const guestStatusArr = statuses.map(item => item.guestStatus)

	if (includes(guestStatusArr, 'stay')) {
		return { guestStatus: 'stay', guestExactStatus: 'stay' };
	}
	if (includes(guestStatusArr, 'arr') && includes(guestStatusArr, 'dep')) {
		return { guestStatus: 'da', guestExactStatus: 'da' };
	}

	return last(statuses);
}

export const getColorBasedOnMemberShipValue = (val) => {
	switch (val?.toLowerCase()) {
	  case 'silver':
		return silverImage;
	  case 'turquoise':
		return turquoiseImage;
	  case 'platinium':
		return platiniumImage;
	  case 'gold':
		return goldImage;
	  case 'platinum':
		return platiniumImage;
	  default:
		return ;
	}
  };
  

  export const getWidthOfImageBaedOnCondition = (val) => {
	switch (val?.toLowerCase()) {
	  case 'silver':
		return 50;
	  case 'turquoise':
		return 80;
	  case 'platinium':
		return 70;
	  case 'gold':
		return 50;
	  case 'platinum':
		return 70;
	  default:
		return ;
	}
  };

  export const getViewOfImageBaedOnCondition = (val) => {
	let membership = val?.toLowerCase()

	if (membership === 'silver' || membership === 'turquoise' || membership === 'platinium' || membership === 'platinum' || membership === 'gold' ) {
		return(
			<View style={styles.SpecialInfoContainer}>
			    <Image style={{ height: 30, width: getWidthOfImageBaedOnCondition(val) }} source={getColorBasedOnMemberShipValue(val)} resizeMode="contain" />
		    </View>
		)
	}else{
		return <></>
	}
  };

  export const getRoomViewOfImageBaedOnCondition = (val) => {
	let membership = val?.toLowerCase()

	if (membership === 'silver' || membership === 'turquoise' || membership === 'platinium' || membership === 'platinum' || membership === 'gold' ) {
		return(
			<View style={styles.membershipRoomListContainer}>
			<Image style={{ height: 30, width: getWidthOfImageBaedOnCondition(membership) }} source={getColorBasedOnMemberShipValue(membership)} resizeMode="contain" />
		  </View>
		)
	}else{
		return <></>
	}
  };

  const styles = StyleSheet.create({
	SpecialInfoContainer: {
		// marginLeft: 5,
		marginRight: 5,
		alignItems: "center",
		justifyContent: "center",
		paddingHorizontal:10,
		backgroundColor:"white",
		shadowColor: "gray",
		shadowOffset: { width: 3, height: 0 },
		shadowOpacity: 0.1,
		shadowRadius: 1,
	  },
	membershipRoomListContainer :{
		marginLeft: 6,
		alignItems: "center",
		justifyContent: "center",
	}
  })
  
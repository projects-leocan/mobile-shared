...
export const createObjectForAddDataInDataDogApiCall = (endpoint, apiCallStatus, options) => {
  const finalObject = {
    endpoint,
    apiCallSuccess: apiCallStatus,
    requestPayload: apiCallStatus === null && options?.body && options?.method && { body: parseBodyFunction(options), method: options?.method }
  }
  return finalObject
}
...
export function request(url, options) {
  ...
  return timeout(fetch(url, options))
    ...
    .then(parseJSON)
    .then(data => {
      writeFileIntoLocal(createObjectForAddDataInDataDogApiCall(url, true))
      return data;
    })
    .catch(err => {

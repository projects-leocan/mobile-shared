...
import {
  ...
  userSelector,
  hotelNameSelector,
  hotelGroupKeySelector,
  hotelhotelGroupnameSelector,
  timeOutTimeSelector,
  networkSelector
} from "../selectors/auth";
import { call, select } from "redux-saga/effects";
import { createObjectForAddDataInDataDogApiCall, writeFileIntoLocal } from 'rc-mobile-base/lib/newLogger';
...
export function request(url, options, timeOutTime) {
  const timeout = timeoutPromise(timeOutTime === undefined ? 24000 : timeOutTime);

  // writeFileIntoLocal(createObjectForAddDataInDataDogApiCall(url, null, options))
  return timeout(fetch(url, options))
    .then(checkStatus)
    .then(parseJSON)
    .then(data => {
      // writeFileIntoLocal(createObjectForAddDataInDataDogApiCall(url, true, options, data))
      return data;
    })
    .catch(err => {
      // writeFileIntoLocal(createObjectForAddDataInDataDogApiCall(url, false))
      if (err === 304) {
        return;
      }
...
export const authRequest = function* authRequest(url, options) {
  ...
  const hotel_group_key = yield select(hotelhotelGroupnameSelector);
  getTimeOutTime = yield select(timeOutTimeSelector);
  const timeOutValue = parseInt(getTimeOutTime.timeOutTime)
...
export const effectRequest = (url, options, meta) => {
  const { auth: { data: { token, user, api, userId, hotelName, hotelGroupname } } } = meta;
  ...
  const timeOutTime = parseInt(getTimeOutTime.timeOutTime)
  const timeout = timeoutPromise(timeOutTime === undefined ? 24000 : timeOutTime);

...
export const authRequest = function* authRequest(url, options) {
  ...
  const hotel_group_key = yield select(hotelGroupKeySelector);
...
export const effectRequest = (url, options, meta) => {
  const { auth: { data: { token, user, api, userId, hotelName, hotelGroupKey } } } = meta;

...
class Socket {
  constructor(dispatch, options) {
    ...
    this._debounceRoomUpdateLong = debounce((socketData, socket, apiEndpoint) => {
      this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "Update inspection status or cleaning status", _.isEmpty(apiEndpoint) ? "" : apiEndpoint))
  ...
  _handlePusherConnected(socket) {
    //writeFileIntoLocal(createObjectForAddDataInDataDogSocketFire(socket))
    this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for internet connected", ""))
    this.dispatch(RoomsActions.socketFireTime())
  ...
  _handlePusherDisconnected(socket) {
    //writeFileIntoLocal(createObjectForAddDataInDataDogSocketFire(socket))
    this.this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for internet disconnected", ""))
    this.dispatch(RoomsActions.socketFireTime())

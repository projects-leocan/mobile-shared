...
// let dateTime = moment().format("DD/MM hh:mm")
const dateTime = new Date().toLocaleString()
// const currentDateTime = new Date()
// const dateTime = moment(currentDateTime).format('YYYY-MM-DD hh:mm:ss');
...
class Socket {
  constructor(dispatch, options) {
    ...
    this.isInspector = get(options, 'isInspector', false)

    this._debounceRoomUpdateLong = debounce((socketData, socket, apiEndpoint) => {
      let roomId = !_.isEmpty(socketData) ? socketData.rids : ""
      this.dispatch(RoomsActions.addSocketList(roomId.toString(), "", _.isEmpty(socket) ? "" : socket, "Update inspection status or cleaning status", ""))
      this.dispatch(RoomsActions.socketFireTime())
      this.dispatch(RoomsActions.roomsFetch(socketData))
    }, 1000)

    // this._debounceRoomUpdateShort = debounce((socketData) => {
    //   // this.dispatch(RoomsActions.roomsFetch(socketData))
    //   this.dispatch(RoomsActions.roomHousekeepingsFetch(socketData))
    // }, 1000)

    this._debounceRoomUpdateShort = (socketData, socket, apiEndpoint) => {
    let roomId = !_.isEmpty(socketData) ? socketData.rids : ""
     this.dispatch(RoomsActions.addSocketList(roomId.toString(), "", _.isEmpty(socket) ? "" : socket, "Update inspection status or cleaning status", ""))
      this.dispatch(RoomsActions.socketFireTime())
    ...
    this._debounceHost = debounce((socket) => {
      this.dispatch(RoomsActions.planningsHostInFetch());
      this.dispatch(RoomsActions.planningsHostOutFetch());
      //this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "", ""))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  activate(userId, hotelGroupId) {
    ...
    channel.bind('reservations_changed', (data) => this._handleResrvationUpdate('reservations_changed', data), this);
  ...
  _handleRoomUpdate = async (socket, data) => {
    ...
    if (!(isEnableWSBlock && data && get(data, 'session.user._id') === this.userId)) {
      socketApiCallConditions = ['/Room/GetListOfRoomHousekeepings']
    ...
    if (!Array.isArray(configureStore)) {
      ...
      if (subcriptedValue === false) {
          return this.dispatch(RoomsActions.isSocketFireOnUpdateInspection({ isFire: true })), this._debounceRoomUpdateShort(socketData, socket, socketApiCallConditions);
        // return this.dispatch(RoomsActions.isSocketFireOnUpdateInspection({ isFire: true })), this._debounceRoomUpdateLong(socketData);
      }
    }
    if (isEnableWSBlock && data && get(data, 'session.user._id') === this.userId) {
      return  this._debounceRoomUpdateLong(socketData, socket, socketApiCallConditions);
    }

    return this._debounceRoomUpdateShort(socketData, socket, socketApiCallConditions);
  }

  _handleResrvationUpdate = async (socket, data) => {
    let socketData = data ? data : {}
    ...
    InteractionManager.runAfterInteractions(() => {
      this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room reservation when reservation update", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
      this.dispatch(RoomsActions.calendarFetch(socketData));
    });
  }
  ...
  _handleHotelConfig(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      //this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "",  _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleHotelSettings(socket) {
    ...
    if (isEnableHotelSetting) {
      ...
      socketApiCallConditions = ['/AppConfiguration/GetAttendantMobileAppConfigurationForHotel']
    }
    //writeFileIntoLocal(createObjectForAddDataInDataDogSocketFire(socket, null, socketApiCallConditions))
    ...
    InteractionManager.runAfterInteractions(() => {
      //this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for get configuration of hotel", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleFloor(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      //this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for get hotel floor", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleCalendar(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room reservation", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleGlitch(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      //this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for get experiences", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handlePlanning(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
     this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room cleanings", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleNightPlanning(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room night cleanings", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
      this.dispatch(RoomsActions.planningsFetch());
    });
  }
  ...
  _handleFuturePlanning(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room future cleanings", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handlePlanningNight(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
     this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get room night cleanings", ""))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handlePlanningRunner(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
      this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get runner room cleaning", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleTask(socket, data) {
    ...
    InteractionManager.runAfterInteractions(() => {
     ...
     this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get hotel tasks", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
    });
  }
  ...
  _handleFutureTask(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
     this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get hotel future tasks", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleCustomAction(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
     // this.dispatch(RoomsActions.addSocketList(_.isEmpty(socket) ? "" : socket, "for get asset actions", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleHostPlanning(socket) {
    ...
    return this._debounceHost(socket);
  }
  ...
  _handleRoomMessages(socket) {
    ...
    InteractionManager.runAfterInteractions(() => {
     ...
     this.dispatch(RoomsActions.addSocketList("", "", _.isEmpty(socket) ? "" : socket, "for get all messages", _.isEmpty(socketApiCallConditions) ? "" : socketApiCallConditions))
      this.dispatch(RoomsActions.socketFireTime())
    });
  }

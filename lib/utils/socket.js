...
class Socket {
  constructor(dispatch, options) {
    ...
    const addSocketData = (socket, description) =>{
      const data = {
        socket: socket,
        description: description
      }
      this.dispatch(RoomsActions.addSocketList(data))
    }

    this._debounceRoomUpdateLong = debounce((socketData, socket) => {
      this.dispatch(RoomsActions.addSocketList(socket, "Update inspection status or cleaning status"))
    ...
    this._debounceRoomUpdateShort = (socketData, socket) => {
      this.dispatch(RoomsActions.addSocketList(socket, "Update inspection status or cleaning status"))
      this.dispatch(RoomsActions.socketFireTime())
  ...
  _handleRoomUpdate = async (socket, data) => {
    ...
    if (!Array.isArray(configureStore)) {
      ...
      if (subcriptedValue === false) {
          return this.dispatch(RoomsActions.isSocketFireOnUpdateInspection({ isFire: true })), this._debounceRoomUpdateShort(socketData, socket);
        // return this.dispatch(RoomsActions.isSocketFireOnUpdateInspection({ isFire: true })), this._debounceRoomUpdateLong(socketData);
      }
    }
    if (isEnableWSBlock && data && get(data, 'session.user._id') === this.userId) {
      return  this._debounceRoomUpdateLong(socketData, socket);
    }

    return this._debounceRoomUpdateShort(socketData, socket);
  }
  ...
  _handlePusherDisconnected(socket) {
    //writeFileIntoLocal(createObjectForAddDataInDataDogSocketFire(socket))
    this.this.dispatch(RoomsActions.addSocketList(socket, ""))
    this.dispatch(RoomsActions.socketFireTime())

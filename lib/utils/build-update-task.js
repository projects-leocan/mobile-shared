...
export default function (task, uuid, status, userId, hotelId) {
  ...
  if (isString(status)) {
    ...
    if (status === "paused" || status === "resume") {
      optimistic['is_paused'] = !task[`is_paused`];
      optimistic['is_resume'] = !task[`is_resume`];
    }
    if (status === "resume") {
      optimistic['is_started'] = true
    }
    if (status === "claimed") {

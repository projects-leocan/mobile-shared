...
export default function(assignments, users, groups, isConcierge = false) {
  const meta = {}, assigned = {};
  const labels = [], ids = [];
  const indexedUsers = keyBy(users, 'id');
  const indexGroups = keyBy(groups, 'id');
  const groupUsers = [];

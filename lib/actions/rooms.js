...
export function calendarFetch(socketData) {
  return {
...
export function singleCalendarSuccess(reservation){
  return{
    type: RoomsTypes.SINGLE_CALENDER_SUCCESS,
    reservation,
  } 
}

export function removeReservation(results) {
  return {
    type: RoomsTypes.REMOVE_RESERVATION,
    results,
  }
}
...
export function auditTasksFetch(id) {
  return {
    type: RoomsTypes.AUDIT_TASKS_FETCH,
    meta: {
      debounce: {
        time: 1000
      }
...
export function auditTasksSuccess(tasks) {

  return {
    type: RoomsTypes.AUDIT_TASKS_SUCCESS,
    tasks
  }
}

export function removeCancelTask(cancelTasks) {
  return {
    type: RoomsTypes.CANCEL_TASKS,
    cancelTasks
  }
}
...
export function socketFireTime() {
  let localeDate = new Date()
  let convertedTime = moment.utc(localeDate).local()
  let formatedTime = convertedTime.format('DD-MM HH:mm');
  
  return {
    type: RoomsTypes.SOCKET_FIRE_DATE_TIME,
    payload: {
...
export function addSocketList(roomName = "", code="", socket, description, apiEndpoint){
  let localeDate = new Date()
  let convertedTime = moment.utc(localeDate).local()
  let formatedTime = convertedTime.format('DD-MM HH:mm:ss');
  
  let obj = {roomName: roomName, code:code, socket:socket, description:description, dateTime:formatedTime, api: apiEndpoint}
  return {
    type: RoomsTypes.SOCKET_LIST,
    payload: {
      socketData:obj, 
    }
  }
...
export function socketFireLoader(isLoading) {
  return {
    type: RoomsTypes.SOCKET_FIRE_LOADER,
    isLoading
  }
}

export function fetchRoomLoader(isRoomLoading){
  return{
    type: RoomsTypes.FETCH_ROOM_LOADER,
    isRoomLoading
  }
}

...
export function taskCreateMessage(messagePayload) {
  const tapTs = moment().unix();
  const outboundLabel = `Task create message`;
...
export function inventoryScheduleAssetConsumption(assetConsumationPayload) {
  const outboundLabel = "Inventory asset consumation";
...
export function inventoryConfirmAssetConsumption(confirmAssetPayload) {
  const outboundLabel = "Inventory confirm asset consumation";
  ...
  return {
    ...
    meta: {
      offline: {
        ...
        commit: { type: OutboundTypes.OUTBOUND_INVENTORY_CONFIRM_ASSET_CONSUMATION_SUCCESS },

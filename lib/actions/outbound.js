    ...
    meta: {
      offline: {
        effect: {
          ...
          options: {
            method: 'POST',
            body: { attendantStatus: status, tapTs, cleaningId }
          }
        },
...
export function roomRefuse(roomId, cleanListRoom, headerObj) {
  const status = 'refuse';
  const tapTs = "0";
  const outboundLabel = "Room refuse";
  const { id: cleaningId } = cleanListRoom;
  // const headerData = getHeader();
  const { token, hotel_group_key } = headerObj;
  ...
  return {
    ...
    meta: {
      offline: {
        effect: {
          // url: `${ROOM_UPDATE_API}`,
          url: `https://test-next.roomchecking.com/mobile-api/Cleaning/UpdateCleaningStatus`,
          options: {
            method: 'POST',
            body: { attendantStatus: status, tapTs, cleaningId },
            headers: { Authorization: `Bearer ${token}`, hotel_group_key: hotel_group_key }
          }
        },
        commit: { type: OutboundTypes.OUTBOUND_ROOM_UPDATE_SUCCESS },

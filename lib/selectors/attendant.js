...
const setHotelTaskPopup = (data, hotelRooms, availHotels) => {
  ...
  const newData = data.map((d) => {
    return {
      ...
      is_guest_request: d?.isGuestRequest,
      comment: d?.comment,
      starts_at_string: d?.startsAtString,
      created_at: d?.createdAt,
      created_at_string: d?.startsAtString,
      guest_info: {
        status: d?.status
      },
      assigned: {
      ...
      roomName: get(find(hotelRooms, ['id', get(d, 'roomId', '')], {}), 'name', ''),
      hotelName: get(find(availHotels, ['hotelId', get(d, 'hotelId', '')], {}), 'hotelName', ''),

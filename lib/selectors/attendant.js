...
export const messaegSocketSelector = state => state.socketRooms.socketHotelMessages;
...
const setHotelTaskPopup = (data, hotelRooms, availHotels) => {
  ...
  const newData = data.map((d) => {
    ...
    return {
      ...
      guest_info: {
        guest_name: get(d, 'guest_info.guest_name', null),
      },
    }
  })
...
const setMessageDataInPopup = (room, message) => {
  const finalMessage = message?.map((msg) => {
    const roomIds = msg?.messageRooms?.map(item => item?.roomId);
    return {
      ...msg,
      room: roomIds?.map(item => find(room, { id: item })),
      displayPopup: roomIds?.some(item => some(room, { id: item })) ? true : false
    }
  })
...
export const getMessageWithRoomName = createSelector(
  [roomsSelector, messaegSocketSelector],
  setMessageDataInPopup
)
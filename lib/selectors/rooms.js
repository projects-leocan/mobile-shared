...
export const roomsIdSelector = state => state.rooms.hotelRoomsId;
...
export const calendarIdSelector = state => state.rooms.hotelCalendarId;
export const planningsSelector = state => state.rooms.hotelPlannings || [];
export const futurePlanningsSelector = state => state.rooms.hotelFuturePlannings || [];
...
export const futureTasksSelector = state => state.rooms.hotelFutureTasks;
...
export const auditsSelector = state => state.rooms.audits;
...
export const userPlanningsCopy = createSelector(
  [userSelector, planningsSelector, planningsRunnerSelector],
  (user, plannings, runnerPlannings) => {
    const { id: userId } = user;

    if (user.isAttendant) {
      return plannings.filter(p => p.secondary_user_id == userId);
    } else if (user.isRoomRunner) {
      return runnerPlannings.filter(p => p.secondary_user_id == userId);
    }
...
export const userFuturePlannings = createSelector(
  [userSelector, futurePlanningsSelector],
  (user, futurePlannings) => {
    const { id: userId } = user;

    return futurePlannings.filter(p => p.planning_user_id == userId);
  }
)

export const userFuturePlanningsCopy = createSelector(
  [userSelector, futurePlanningsSelector],
  (user, futurePlannings) => {
    const { id: userId } = user;

    return futurePlannings.filter(p => p.secondary_user_id == userId);
  }
)
...
export const getComputedRooms = (
  ...
  each(hotelRooms, room => {
    ...
    if (roomPlanning && roomPlanning.planning_user_id) {
      plannedUser = indexUsers[roomPlanning.planning_user_id] || {};
    }

    const messageForCurrent = find(indexMessage, function (o) { return o.room_id === roomId; })

    const messagesForRoom = filter(hotelMessages || [], ['isMessageForReservation', false]);
    const roomMessageForCurrent = filter(messagesForRoom, function (item) {
      const messagesRoomId = flatten(get(item, 'messageRooms', []).map(el => el.roomId));
      return messagesRoomId.includes(roomId);
    })
    if (isPlanned && get(roomPlanning, 'planning_user_id') !== userId && get(roomPlanning, 'secondary_user_id') !== userId) {
      return;
    }
    ...
    const excludeFutureCalender = excludeFutureArrivals(get(indexReservations, [roomId, 'roomCalendar'], []));
    ...
    mappedRooms.push({
      ...room,
      floor: get(indexFloors, floorId, {}),
      roomStatus: get(indexRoomStatuses, roomId, {}),
      roomHousekeeping: get(indexRoomHousekeepings, roomId, {}),
      // roomCategory: get(indexRoomCategories, roomCategoryId, {}),
      roomCategory,
      roomPlanning,
      ...
      guestExactStatus: get(guestStatus, 'guestExactStatus', null),
      roomCredits: get(roomPlanning, 'credits') || get(room, 'overwriteCredits') || get(room.roomCategory, 'credts') || 1,
      sortValue: get(room, 'sortValue') || roomsSorting(get(room, 'name')),
      update: get(roomsUpdates, roomId, null),
      roomMessage: get(messageForCurrent, 'messageHolder', null),
      roomMessageWithObject: isEmpty(roomMessageForCurrent) ? null : roomMessageForCurrent,
      plannedUser
    });
  });
...
export const getAvailableFloors = (hotelRooms, indexFloors) => {
  ...
  const mapped = map(usedFloorIds, (floorId, index) => {
    ...
    const sortedRooms =  _.sortBy(rooms, [function(item) { return item?.name; }]);
    const floorName = first(uniq(compact(flatten(map(rooms, 'floorName'))), true))
    const buildingName = first(uniq(compact(flatten(map(rooms, 'buildingName'))), true));
...
export const getAvailableAllFloors = (hotelRooms, indexFloors) => {
  if (!hotelRooms || !hotelRooms.length || isEmpty(indexFloors)) {
    return [];
  }
...
export const getTasks = (hotelTasks, indexUsers) => {
  ...
  return hotelTasks.map(task => {
    const mapResponsibleUsers = map(get(task, 'assigned.user_ids', []), function (userId) {
      return get(indexUsers, userId)
    })
    return extend({}, task, {
      creator: task.creator_id && get(indexUsers, task.creator_id),
      responsible: task.responsible_id && get(indexUsers, task.responsible_id),
      allResponsible: mapResponsibleUsers
    });
  })
}
...
export const getPopupTasks = (hotelTasks, hotelRooms, userId, availHotels) => {
  if (!hotelTasks || !hotelTasks.length) {
    return [];
  }
...
export const getAllFilteredRooms = (hotelRooms, filters) => {
  if (!hotelRooms || !hotelRooms.length) {
    return [];
  }

  if (!filters.isActiveAllFilter) {
    return hotelRooms;
  }

  let filtered = hotelRooms;
  if (filters.roomsSearchQueryAll) {
    const cleanQuery = filters.roomsSearchQueryAll.toLowerCase();
    filtered = filtered.filter(room => {
      const matchesGuest = get(room, 'roomCalendar', []).reduce((pv, entry) => {
        if (get(entry, 'guest_name', '').toLowerCase().includes(cleanQuery)) {
          pv = true;
  ...
  //   });
  // }
  if (filters.activeAllRooms && filters.activeAllRooms.length) {
    filtered = filtered.filter(room => {
      return filters.activeAllRooms.includes(room.id);
    });
  } else if (filters.activeRooms.length <= 0) {
    const cleanQuery = filters.roomsSearchQueryAll.toLowerCase();
    if (!cleanQuery) {
...
const generateFilterArrays = (rooms, allRooms) => {
  let filter1Status = [
  ...
  const result = rooms.map((room, index) => {
    ...
    const roomFrontOfficeStatus = get(room, 'roomStatus.frontOfficeStatus');
    const roomHouskeepingStatus = get(room, 'roomStatus.houskeepingStatus');
    const roomStatusCode = get(room, 'roomStatus.code')
    const roomGuests = get(room, 'guests', [])

    // if (roomStatusCode === "DPE") {
    //   filter1Status['departure'].rooms.push(room);
    // }
    if (roomStatusCode === "DPD") {
  ...
  return compact(result, arrivedRooms)
}
...
export const getInspectorFilteredRooms = (hotelRooms, filters) => {
  ...
  let roomFilterData = first(generateFilterArrays(privateRoom, privateRoom));

  if (!filters.isActiveFilter && !filters.isHeaderFilter) {
    const resultPayload = {
  ...
  const cleanerFilterOperation = get(filterOperation, 'cleanerSelectionOperation.isSelectCleanerFilter', false);
  const isNightyCleaningTrue = get(filterOperation, 'cleanerSelectionOperation.isNightyCleaning', false);
  const attendantFilter = get(filterOperation, 'attendantSelectedFilter', [])
  const attendantNightyFilter = get(filterOperation, 'attendantSelectedFilterNight', [])
  ...
  if (isroomTypeAll) {
    ...
    filtered = uniqBy(flatten(roomTypeOperation.map((roomType) => handleRoomTypeData(roomType, hotelRooms)), ''), 'id');
  }
  ...
  if (filters.roomsSearchQuery) {
    ...
    filtered = filtered.filter(room => {
      ...
      const data = matchesGuest || room.name.toLowerCase().includes(cleanQuery);
  ...
  if (get(floorSectionOperation, 'length') > 0) {
    const filteredBySection = filtered.filter(room => {
  ...
  if (get(roomOperation, 'length') > 0) {
    try {
      roomFilterData = first(generateFilterArrays(filtered, privateRoom));
      let mergeData = []
      let selectedData = map(roomOperation, function (obj) {
        ...
        if (get(obj, 'rootType') === 1) {
          ...
          if ((get(obj, 'rootType') === 1 && get(obj, 'filterType') === 1)) {
            if (get(obj, 'selectedIndex') === 0 || get(obj, 'selectedIndex') === 2 || get(obj, 'selectedIndex') === 3) {
              mergeData.push(...selectionRooms)
            }else{
              return selectionRooms;
            }
          }
          else if ((get(obj, 'rootType') === 1 && get(obj, 'filterType') === 3)) {
            if (get(obj, 'selectedIndex') === 2 || get(obj, 'selectedIndex') === 3) {
              mergeData.push(...selectionRooms)
            }else{
              return selectionRooms;
            }
      ...
      //remove undefined array element from selected data
      selectedData = _.compact(selectedData);
    
      if (_.isEmpty(selectedData)) {
        // arrived+arrivals has data and other filter is empty
        filtered = mergeData
      }else if (_.isEmpty(mergeData)){
        // arrived+arrivals is empty and other filter has data
        filtered = _.intersection.apply(_, selectedData)
      }
      else{
        // arrived+arrivals has data and other filter has data
        selectedData.push(mergeData)
        filtered = _.intersection.apply(_, selectedData);
      } 
    } catch (error) {
  ...
  if (cleanerFilterOperation) {
    if (isNightyCleaningTrue) {
      filtered = filtered.filter(room => {
        return !_.isEmpty(get(room, 'roomNightPlanning'))
      });
    }
    else {
      filtered = filtered.filter(room => {
        return !_.isEmpty(get(room, 'roomPlanning'))
      });
    }
  }

  if (!isNightyCleaningTrue && !_.isEmpty(attendantFilter)) {
    filtered = filtered.filter(room => {
      return attendantFilter.includes(get(room, 'roomPlanning.planning_user_id', null))
    });
  }

  if (!_.isEmpty(attendantNightyFilter) && isNightyCleaningTrue) {
    filtered = filtered.filter(room => {
      return attendantNightyFilter.includes(get(room, 'roomNightPlanning.planning_user_id', null))
    });
  }
...
export const computedHotelTasks = createDeepEqualSelector(
  [availHotelsTaskSelector, computedIndexUsers],
  // makeProfile(getTasks, 'getTasks')
  getTasks
);
...
export const computedUserTasks = createSelector(
  [computedHotelTasks, userIdSelector],
  getUserTasks
);

export const computedUserHotelsTasks = createSelector(
  [computedTasks, userIdSelector],
  getUserTasks
);
...
export const computedHotelsPopupTasks = createSelector(
  [computedUserHotelsTasks, roomsSelector, userIdSelector, availableHotelsSelector],
  getPopupTasks
);
...
export const computedFilteredRooms = createSelector(
  [computedHotelRooms, filtersSelector],
  getFilteredRooms
);

export const computedFilteredFloors = createSelector(
  [computedFilteredRooms, computedIndexFloors],
  getAvailableFloors
)
...
export const nightyPlanningsForUser = createSelector(
  [planningsNightSelector, userIdSelector],
  (plannings, userId) => plannings.filter(p => p.planning_user_id === userId)
)

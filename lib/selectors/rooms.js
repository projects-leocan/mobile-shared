...
const generateFilterArrays = (rooms) => {
  ...
  const result = rooms.map((room, index) => {
    ...
    const roomFilterStatus = !_.isEmpty(roomGuests) && roomGuests.map(item => item.status)
    ...
    if (includes(roomFilterStatus, 'departed') && includes(roomFilterStatus, 'arrival')) {
      filter3Status['vacant'].rooms.push(room);
    }

    if (roomGuestStatus === 'arrival' || roomGuestStatus === 'departed') {
      filter3Status['vacant'].rooms.push(room);
    }
    ...
    if (roomStatusCode === 'OCC') {
     ...
     else if (roomStatusCode === 'VAC' && !(roomGuestStatus === 'stay' || roomGuestStatus === 'arrived')) {
      filter3Status['vacant'].rooms.push(room);
    }

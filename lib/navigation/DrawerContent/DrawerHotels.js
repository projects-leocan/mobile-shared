...
import {filterAssetActionOnHotelChange} from "rc-mobile-base/lib/actions/assets"
import { get } from 'lodash';
import { computedHotelsPlanning } from 'rc-mobile-base/lib/selectors/rooms';
...
class DrawerHotels extends React.Component {
    ...
    onNavigateToTask = (hotelId) => {
        const { activeAvailHotel, navigation, screenProps, selectedHotel, handleHotelChange , filterAssetActionOnHotelChange} = this.props;
        if(selectedHotel !== hotelId) {
            activeAvailHotel(hotelId);
            filterAssetActionOnHotelChange(hotelId)
        }
    ...
    renderHotelRow = (item, index) => {
        ...
        const availablePlanningForItem = get(item, 'hotelsPlanning', []);
        const availablePlanningLength = get(availablePlanningForItem, 'length')
        ...
        return (
            <TouchableOpacity style={styles.itemCellContainer} activeOpacity={0.7} onPress={() => this.onNavigateToTask(hotelId)} >
                ...
                <View style={styles.hotelNameContainer}>
                    ...
                    {availablePlanningLength > 0 ?
                    <View style={styles.cleaningImageView}>
                        <Image resizeMode={'contain'} source={require("../../images/attendant_cleaning.png")}/>
                    </View> : <></>
                    }
                </View>
    ...
    render() {
        const { handleHotelChange, hotelsTask, allHotelPlanning } = this.props;
...
const mapStateToProps = state => {
    return {
        ...
        planning: state.rooms.hotelPlannings,
    }
}
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        filterAssetActionOnHotelChange: hotelId => dispatch(filterAssetActionOnHotelChange(hotelId)),
        dispatch
    }
};

...
import { computedHotelsTask, getComputedHotelsTask, hotelsTaskSelector } from 'rc-mobile-base/lib/selectors/hotelsTask';
...
class DrawerHotels extends React.Component {
    ...
    renderHotelRow = (item, index) => {
        const { selectedHotel, hotelsTask } = this.props;
        const availableTaskForItem = get(item, 'hotelsTask', []);

        return (
            <TouchableOpacity style={styles.itemCellContainer} activeOpacity={0.7} >
                <View style={[styles.hotelImageContainer, { backgroundColor: selectedHotel === item.hotelId ? splashBg.color : themeTomato.color }]}>
    ...
    render() {
        const { availableHotels, handleHotelChange, hotelsTask, selectedHotel } = this.props;
...
const mapStateToProps = state => {
    return {
        ...
        selectedHotel: hotelIdSelector(state)
    }
}

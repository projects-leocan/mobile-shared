...
import { Back, Hamburger, TaskButton } from './helpers';
...
const navigationCreate = (scenes, links, params) => {
  ...
  if (MainLayout) {
    MainLayout.navigationOptions = ({ navigation, screenProps }) => ({
      ...
      headerRight: <TaskButton tasksLength={screenProps.tasksLength || 0} onPress={() => navigation.navigate("Tasks")} />
    });
  }

  if (TasksLayout) {
    TasksLayout.navigationOptions = ({ navigation, screenProps }) => ({
      headerRight: screenProps.isAttendantApp ? <TaskButton tasksLength={screenProps.tasksLength || 0} onPress={() => console.log('navigate to task')} /> : null
    });
  }
